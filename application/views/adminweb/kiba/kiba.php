...
$( "#search" ).change(function() {
	var t = $('#search').val();
	 if (t != "all" && t != ""){    		
										...
										$.ajax({
												type: "POST",
												url : "<?php echo site_url('kiba/select_q')?>",
												data: "q="+t,
...
<script>
	$(function() {
		...
		kd_upb: $("#kd_upb").val(),
				  ...
				  $.ajax({
					url : "<?php echo site_url('kiba/pindahskpd')?>",
					type : 'POST',
					data : kode,
					success: function(msg){
...
<!-- start usul guna  data ->
	...
	if(confirm("Data ini akan diusulkan untuk penetapan status penggunaan barang ??"))
	{
		...
		$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>kiba/usul_guna",
				data:kode,
			 	cache: false,
...
<!-- end usul guna data ->			
	...
	if(confirm("Yakin data ini mau di Hapus ??"))
	{
		...
		$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>kiba/ajax_hapus",
				data:kode,
			 	cache: false,
			 	success: function(html){
...
$(".aktifkan_semua").click(function(event) {
	if(confirm("Yakin data ini sudah benar dan akan diaktifkan ??")){
		...
		$("input.checkbox1:checked").each(function(){
			...
			$.ajax({
					type: "POST",
					url: "<?php echo base_url(); ?>kiba/aktifkan",
					data:kode,
					cache: false,
					success: function(html){
						$("#listhapus_"+no).fadeTo(300,0, function() {
...
$(".non_operasional").click(function(event) {
	...
	if(confirm("Ubah status operasional ??"))
	{
		...
		$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>kiba/operasional",
				data:kode,
			 	cache: false,
			 	success: function(html){
...
$(function() {
		...
		var page = "<?php echo site_url('kiba/add_usul_hapus')?>";
		var $dialog = $('<div></div>')
		               // .html('<iframe style="border: 0px; " src="' + page + '" width="100%" height="100%"></iframe>')
   		               .load(page)
		               .dialog({
		    title:"Usul Penghapusan Barang KIB A. Tanah",
		    modal: true,
		    draggable: false,
		    resizable: false,
		    position: ['center'],
			...
			buttons: {
				"Proses": function() {
				  ...
				  $.ajax({
					url : "<?php echo site_url('kiba/proses_usul_hapus')?>",
					type : 'POST',
					data : kode,
					success: function(msg){
							// alert(msg+"data berhasil diusulkan sebagai daftar usulan penghapusan!");
							$dialog.dialog('close');
...
$(function() {
		...
		var page = "<?php echo site_url('kiba/add_pindah_skpd')?>";
		var $dialog = $('<div></div>')
		               // .html('<iframe style="border: 0px; " src="' + page + '" width="100%" height="100%"></iframe>')
   		               .load(page)
		               .dialog({
			...
			buttons: {
				"Proses": function() {
				  ...
				  $.ajax({
					url : "<?php echo site_url('kiba/proses_pindah_skpd')?>",
					type : 'POST',
					data : kode,
					success: function(msg){
                ...
                <div class="panel">
             	...
             	<div class="panel-content list-content">
                        ...
                        <ul class="nav nav-tabs">
                            ...
                            <a href="<?php echo ! empty($tab1) ?  $tab1: '#'; ?>" class="tab-page" data-toggle="tab">DATA KIB A. TANAH
                       ...
                       <div class="tab-content">
                            <div class="tab-pane list-pane active" id="tab1">
                                ...
                                <div class="pull-right">
                                    <form class="form-inline" method="POST" id="form_view" action=<?php echo base_URL(); ?>kiba/set/>
                                     ...
                                     <button class="btn"><i class="icon-search icon-large"></i></button>
                                        <a class="btn" href="<?php echo base_url(); ?>kiba/export"><i class="icon-ok-circle"></i> Excell</a>
                                    </form>
                                </div>
                	...
                	<div class="list-actions">
                            <div class="btn-toolbar">
                                ...
                                <div class="btn-group">
                                    <a href="<?php echo base_url() ?>kiba/add">
                  ...
                  <table class="table">
		       	...
		       	echo "<li>".anchor('kiba/lihat/'.$row->Kd_Prov.'/'.$row->Kd_Kab_Kota.'/'.$row->Kd_Bidang.'/'.$row->Kd_Unit.'/'.$row->Kd_Sub.'/'.$row->Kd_UPB.'/'.$row->Kd_Aset1.'/'.$row->Kd_Aset2.'/'.$row->Kd_Aset3.'/'.$row->Kd_Aset4.'/'.$row->Kd_Aset5.'/'.$row->No_Register,"<i class='icon-eye-open'></i>Lihat",array('class' => 'update'))."</li>";
			...
			if(empty($row->Penghapusan) || ($row->Penghapusan == 3)){
				...
				if($row->Penghapusan <> 3){
					if($this->session->userdata('lvl') == '01' || $this->session->userdata('lvl') == '02'){                                    
						echo "<li>".anchor('kiba/update/'.$row->Kd_Prov.'/'.$row->Kd_Kab_Kota.'/'.$row->Kd_Bidang.'/'.$row->Kd_Unit.'/'.$row->Kd_Sub.'/'.$row->Kd_UPB.'/'.$row->Kd_Aset1.'/'.$row->Kd_Aset2.'/'.$row->Kd_Aset3.'/'.$row->Kd_Aset4.'/'.$row->Kd_Aset5.'/'.$row->No_Register,"<i class='icon-pencil'></i>Ubah",array('class' => 'update'))."</li>";
					}
