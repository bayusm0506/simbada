...
class PDF_MC_Table_penyusutan_satuan extends FPDF
{
...
function Rowheader()
{
		...
		$this->MultiCell(30,5,"Kode Barang",0,'C');

		$this->SetXY($x+55,$y);
		$this->MultiCell(20,15,"",1,'C');
		$this->SetXY($x+55,$y2+2);
		$this->MultiCell(20,5,"No Register",0,'C');

		$this->SetXY($x+75,$y);
		$this->MultiCell(50,15,"",1,'C');
		$this->SetXY($x+75,$y2+2);
		$this->MultiCell(50,5,"Nama / Jenis Barang",0,'C');

		$this->SetXY($x+125,$y);
		$this->MultiCell(25,15,"",1,'C');
		$this->SetXY($x+125,$y2);
		$this->MultiCell(25,5,"Tahun Perolehan",0,'C');

		$this->SetXY($x+150,$y);
		$this->MultiCell(20,15,"",1,'C');
		$this->SetXY($x+150,$y2);
		$this->MultiCell(20,5,"Masa Manfaat",0,'C');

		$this->SetXY($x+170,$y);
		$this->MultiCell(45,15,"",1,'C');
		$this->SetXY($x+170,$y2+3);
		$this->MultiCell(45,5,"Nilai Perolehan",0,'C');

		$this->SetXY($x+215,$y);
		$this->MultiCell(45,15,"",1,'C');
		$this->SetXY($x+215,$y2+3);
		$this->MultiCell(45,5,"Kapitalisasi",0,'C');

		$this->SetXY($x+260,$y);
		$this->MultiCell(45,15,"",1,'C');
		$this->SetXY($x+260,$y2+3);
		$this->MultiCell(45,5,"Beban Penyusutan",0,'C');

		$this->SetXY($x+305,$y);
		$this->MultiCell(45,15,"",1,'C');
		$this->SetXY($x+305,$y2+3);
		$this->MultiCell(45,5,"Akumulasi Penyusutan",0,'C');

		$this->SetXY($x+350,$y);
		$this->MultiCell(25,15,"",1,'C');
		$this->SetXY($x+350,$y2);
		$this->MultiCell(25,5,"Sisa Masa Manfaat",0,'C');

		$this->SetXY($x+375,$y);
		$this->MultiCell(45,15,"",1,'C');
		$this->SetXY($x+375,$y2+3);
		$this->MultiCell(45,5,"Nilai Buku",0,'C');

		$this->SetXY($x+420,$y);
		$this->MultiCell(30,15,"Keterangan",1,'C');
}
...
function ttd2($nm_pimpinan,$nip_pimpinan,$jbt_pimpinan,$nm_pengurus,$nip_pengurus,$tanggal)
{
	...
	function Header()
	{
	    ...
	    $this->Cell(0,9,'DAFTAR PENYUSUTAN PER SATUAN BARANG',0,0,'C');
		$this->Ln();
		$this->SetFont('Arial','B','10');
		$this->SetWidths(array(15,30,20,50,25,20,45,45,45,45,25,45,30));
