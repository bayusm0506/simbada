	...
	$(document).ready(function(){
	      ...
	      { 
			...
			var kode = {
				...
				$.ajax({
									type: "POST",
									url : "<?php echo base_url(); ?>kibc/cek_register",
									data: kode,
...
<script type="text/javascript">
	    $(this).ready( function() {
    		$("#Nm_Aset5").autocomplete({
        		...
        		function(req, add){
          			$.ajax({
		        		url: "<?php echo base_url(); ?>kibc/json",
		          		dataType: 'json',
		          		type: 'POST',
		          		data: req,
		          		success:    
		...
		<script type="text/javascript">			
			$(document).ready(function(){	
				...
				$("#kode-search").click(function(){
					...
					$("#tkodebarang").jqGrid({
					url: '<?php echo base_url(); ?>kibc/lookup',
					datatype: "json", 
					mtype: 'GET',
...
<!-- Validation -->
          ...
          <tr>
            <td>Luas Lantai</td>    <td> : <input type=text name='Luas_Lantai' size=15 value="<?php echo $default['Luas_Lantai']; ?>" id="Luas_M2" class="input-small"></td>
            </tr>
          ...
          <tr>
			...
			echo form_dropdown('Kondisi', $option_kondisi, $default['Kondisi'],' class="input-small"');
		  ?></td>
          </tr>
          <tr>
            <td>Bertingkat</td>
		  ...
		  $option_bertingkat = array('Bertingkat' => 'Bertingkat','Tidak'=>'Tidak');
			...
			echo form_dropdown('Bertingkat_Tidak', $option_bertingkat, $default['Bertingkat_Tidak']);
		  ?> Beton/tidak : 
            <?php
		  $option_Beton = array('Beton' => 'Beton','Tidak'=>'Tidak');
		  
			echo form_dropdown('Beton_tidak', $option_Beton, $default['Beton_tidak']);
		  ?></td>
          </tr>
		  <tr>
		    <td>Tgl Dokumen</td>    <td> : <input type=text name='Dokumen_Tanggal' size=10 
          value="<?php echo tgl_ymd($default['Dokumen_Tanggal']); ?>" id='datepicker3' readonly='readonly'  class="input-small"> 
		      Nomor Dokumen : 
		  ...
		  <tr>
		    <td class='ket'>Masa Manfaat</td>
		    <td>: <input type=text name='Masa_Manfaat' value="<?php echo $default['Masa_Manfaat']; ?>"  class="input-mini"></td>
		    </tr>
		  <tr>
