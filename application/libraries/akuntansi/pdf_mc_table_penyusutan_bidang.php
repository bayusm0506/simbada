...
class PDF_MC_Table_penyusutan_bidang extends FPDF
{
...
function UPBTitle2($kode_lokasi)
{
	...
	function Rowheader()
	{
			...
			$this->SetXY($x+25,$y);
			$this->MultiCell(55,15,"",1,'C');
			$this->SetXY($x+25,$y2+3);
			$this->MultiCell(55,5,"Nama / Jenis Barang",0,'C');

			$this->SetXY($x+80,$y);
			$this->MultiCell(30,15,"",1,'C');
			$this->SetXY($x+80,$y2+3);
			...
			$this->MultiCell(25,15,"",1,'C');
			$this->SetXY($x+135,$y2);
			$this->MultiCell(25,5,"Masa Manfaat",0,'C');

			$this->SetXY($x+160,$y);
			$this->MultiCell(45,15,"",1,'C');
			$this->SetXY($x+160,$y2+3);
			$this->MultiCell(45,5,"Nilai Perolehan",0,'C');

			$this->SetXY($x+205,$y);
			$this->MultiCell(45,15,"",1,'C');
			$this->SetXY($x+205,$y2+3);
			$this->MultiCell(45,5,"Penyusutan Pertahun",0,'C');

			$this->SetXY($x+250,$y);
			$this->MultiCell(45,15,"",1,'C');
			$this->SetXY($x+250,$y2+3);
			$this->MultiCell(45,5,"Akumulasi Penyusutan",0,'C');

			$this->SetXY($x+295,$y);
			$this->MultiCell(45,15,"",1,'C');
			$this->SetXY($x+295,$y2+3);
			$this->MultiCell(45,5,"Kapitalisasi",0,'C');

			$this->SetXY($x+340,$y);
			$this->MultiCell(45,15,"",1,'C');
			$this->SetXY($x+340,$y2+3);
...
function ttd2($nm_pimpinan,$nip_pimpinan,$jbt_pimpinan,$nm_pengurus,$nip_pengurus,$tanggal)
{
	...
	function Header()
	{
	    ...
	    $this->Cell(0,9,'DAFTAR PENYUSUTAN',0,0,'C');
		$this->Ln();
		$this->SetFont('Arial','B','10');
		$this->SetWidths(array(15,55,30,25,25,45,45,45,45,45,25,40));
