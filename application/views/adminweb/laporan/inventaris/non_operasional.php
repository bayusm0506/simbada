<?php
	...
	$pdf->MultiCell(435,5,"DAFTAR ASET NON OPERASIONAL \n ".NM_PEMDA,0,'C');
	...
	foreach ($kibc->result() as $row){
		...
		$harga		= number_format($row->Harga,0,",",".");
		$pdf->Row(array($no, $kodebarang, $register, $row->Nm_Aset5,"-", $row->Dokumen_Nomor, "-", $row->Asal_usul,$row->Tahun, $row->Luas_Lantai,"M2", $kondisi,$row->Jumlah,rp($row->Harga), $row->Keterangan));
		$no++;
	}
	}else{
		$pdf->nihil2();
	...
	$jumlah_total = $jumlah_a + $jumlah_b + $jumlah_c + $jumlah_d + $jumlah_e + $jumlah_l;
	$harga_total  = $harga_a + $harga_b + $harga_c + $harga_d + $harga_e + $harga_l;
	...
	$pdf->Output("SIMDO_Aset_Non_Operasional.pdf","I");
?>