<?php
	...
	$pdf->MultiCell(435,5,"PENETAPAN STATUS PENGGUNAAN BARANG MILIK DAERAH \n ".NM_PEMDA,0,'C');
	...
	$totalkiba		= number_format($total_usulguna_kiba,0,",",".");
	$pdf->Row(array("01", "TANAH", "", "","", "", "", "","", $totalkiba, "", "","", ""));
	if ($jumlah_usulguna_kiba > 0){
		$pdf->SetFont('Times','','10');	
		foreach ($usulguna_kiba->result() as $row){
	...
	$no=1;
	$pdf->SetFont('Times','','10');
	$totalkibb		= number_format($total_usulguna_kibb,0,",",".");	
	$pdf->Row(array("02", "PERALATAN DAN MESIN", "", "","", "", "", "","", $totalkibb, "", "","", ""));		
	if ($jumlah_usulguna_kibb > 0){
		$pdf->SetFont('Times','','10');	
		foreach ($usulguna_kibb->result() as $row){
			$harga		= number_format($row->Harga,0,",",".");
			$kd1 = sprintf ("%02u", $row->Kd_Aset1);
			$kd2 = sprintf ("%02u", $row->Kd_Aset2);
			$kd3 = sprintf ("%02u", $row->Kd_Aset3);
	...
	$no=1;
	$pdf->SetFont('Times','','10');
	$totalkibc		= number_format($total_usulguna_kibc,0,",",".");	
	$pdf->Row(array("03", "GEDUNG & BANGUNAN", "", "","", "", "", "","", $totalkibc, "", "","", ""));		
	if ($jumlah_usulguna_kibc > 0){
		$pdf->SetFont('Times','','10');	
		foreach ($usulguna_kibc->result() as $row){
			$harga		= number_format($row->Harga,0,",",".");
			$kd1 = sprintf ("%02u", $row->Kd_Aset1);
			$kd2 = sprintf ("%02u", $row->Kd_Aset2);
			$kd3 = sprintf ("%02u", $row->Kd_Aset3);
	...
	$no=1;
	$pdf->SetFont('Times','','10');
	$totalkibd		= number_format($total_usulguna_kibd,0,",",".");	
	$pdf->Row(array("04", "JALAN, IRIGASI & JARINGAN", "", "","", "", "", "","", $totalkibd, "", "","", ""));		
	if ($jumlah_usulguna_kibd > 0){
		$pdf->SetFont('Times','','10');	
		foreach ($usulguna_kibd->result() as $row){
			$harga		= number_format($row->Harga,0,",",".");
			$kd1 = sprintf ("%02u", $row->Kd_Aset1);
			$kd2 = sprintf ("%02u", $row->Kd_Aset2);
			$kd3 = sprintf ("%02u", $row->Kd_Aset3);
	...
	$no=1;
	$pdf->SetFont('Times','','10');
	$totalkibe		= number_format($total_usulguna_kibe,0,",",".");	
	$pdf->Row(array("05", "ASET TETAP LAINYA", "", "","", "", "", "","", $totalkibe, "", "","", ""));		
	if ($jumlah_usulguna_kibe > 0){
		$pdf->SetFont('Times','','10');	
		foreach ($usulguna_kibe->result() as $row){
			$harga		= number_format($row->Harga,0,",",".");
			$kd1 = sprintf ("%02u", $row->Kd_Aset1);
			$kd2 = sprintf ("%02u", $row->Kd_Aset2);
			$kd3 = sprintf ("%02u", $row->Kd_Aset3);
