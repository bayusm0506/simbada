<?php
	...
	$this->load->library('perencanaan/PDF_MC_Table_usul_rkpbu');
	
	$pdf=new PDF_MC_Table_usul_rkpbu('L','mm','f4');
	$pdf->Open();
	$pdf->AddPage();
	$pdf->Image(base_url().'asset/img/logo.png',10,5,22,0,'PNG');
	$pdf->SetFont('Arial','B',15);
	$pdf->MultiCell(435,5,"DAFTAR USULAN RENCANA KEBUTUHAN PEMELIHARAAN BARANG \n ".NM_PEMDA,0,'C');
	...
	$pdf->SetWidths(array(15,90,30,45,20,20,30,20,20,20,65,20,20,25));
	$pdf->Rowheader();
	...
	if ($jumlah_rkpbu > 0){
		foreach ($rkpbu->result() as $row){
			...
			$pdf->Row(array($no, $row->Ket_Kegiatan, $kode_barang, $row->Nm_Aset5,"5", "6", "7", "8", "9", "10", $row->Uraian, rp($row->Jumlah), "Unit", $row->Keterangan));
			$no++;
		}
	}else{
		$pdf->nihil();
