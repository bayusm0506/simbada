<?php
	...
	$pdf->MultiCell(435,5,"DAFTAR PENYUSUTAN KIB C. GEDUNG DAN BANGUNAN\nPEMERINTAH PROVINSI SUMATERA UTARA",0,'C');
	...
	foreach ($kib->result() as $row){
		...
		$pdf->Row(array($no,$row->Nm_Aset5.'('.$row->No_Register.')', $kodebarang, $row->Tahun, $row->Masa_Manfaat,  rp($row->Harga), rp($row->Susut),   rp($row->Akm_Susut),   rp($row->Kapitalisasi),  rp($row->NB),   rp($row->Sisa_Masa_Manfaat),  $row->Keterangan));
		$no++;
	}
	}else{
		$pdf->nihil2();
