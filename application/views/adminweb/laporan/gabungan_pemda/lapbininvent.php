<?php
	...
	$this->load->library('PDF_MC_Table_lapbininvent');
	
	$pdf=new PDF_MC_Table_lapbininvent('L','mm','f4');
	$pdf->Open();
	$pdf->AddPage();
	...
	foreach ($kiba->result() as $row){
		...
		$pdf->Row(array($no, $kodebarang, $register, $row->Nm_Aset5,"-", $row->Sertifikat_Nomor, "-", $row->Asal_usul,$row->Tahun, $row->Luas_M2, "M2", "-",$row->Jumlah,$harga, $row->Nm_UPB));
		$no++;
	}
	}else{
		$pdf->nihil2();
	...
	$pdf->Row(array("02", "", "", "PERALATAN DAN MESIN","", "", "", "","", "", "", "",$totalb->Jumlah, $totalkibb,  ""));
	if ($jumlahb > 0){
	foreach ($kibb->result() as $row){
		...
		$harga		= number_format($row->Harga,0,",",".");
		$pdf->Row(array($no, $kodebarang, $register, $row->Nm_Aset5,$merk, "-", $row->Bahan,  $row->Asal_usul,$row->Tahun, $row->CC, '',	$kondisi,$row->Jumlah, $harga, $row->Nm_UPB));		
		$no++;
	}
	}else{
		$pdf->nihil2();
	...
	$pdf->Row(array("03", "", "", "GEDUNG DAN BANGUNAN","", "", "", "","", "", "", "",$totalc->Jumlah, $totalkibc,""));
	if ($jumlahc > 0){
	foreach ($kibc->result() as $row){
		...
		$harga		= number_format($row->Harga,0,",",".");
		$pdf->Row(array($no, $kodebarang, $register, $row->Nm_Aset5,"-", $row->Dokumen_Nomor, "-", $row->Asal_usul,$row->Tahun, $row->Luas_Lantai,"M2", "-",$row->Jumlah,$harga, $row->Nm_UPB));
		$no++;
	}
	}else{
		$pdf->nihil2();
	...
	$pdf->Row(array("04", "", "", "JALAN, IRIGASI DAN JARINGAN","", "", "", "","", "", "", "",$totald->Jumlah, $totalkibd,""));
	if ($jumlahd > 0){
	foreach ($kibd->result() as $row){
		...
		$harga		= number_format($row->Harga,0,",",".");
		$pdf->Row(array($no, $kodebarang, $register, $row->Nm_Aset5,"-", "", "-", $row->Asal_usul,$row->Tahun, $row->Luas,"M2", "-", $row->Jumlah, $harga, $row->Nm_UPB));
		$no++;
	}
	}else{
		$pdf->nihil2();
	...
	$pdf->Row(array("05", "", "", "ASET TETAP LAINYA","", "", "", "","", "", "", "",$totale->Jumlah, $totalkibe, ""));
	if ($jumlahe > 0){
	foreach ($kibe->result() as $row){
		...
		$harga		= number_format($row->Harga,0,",",".");
		$pdf->Row(array($no, $kodebarang, $register, $row->Nm_Aset5,"-", "", "-", $row->Asal_usul,$row->Tahun, '-', "", "-",$row->Jumlah, $harga, $row->Nm_UPB));
		$no++;
	}
	}else{
		$pdf->nihil2();
	...
	$pdf->Row(array("06", "", "", "KONSTRUKSI DALAM PENGERJAAN","", "", "", "","", "", "", "",$totalf->Jumlah, $totalkibf, ""));
	if ($jumlahf > 0){
	...
	$pdf->Row(array("", "", "", "TOTAL ","", "", "", "","", "", "", "",$jumlah, $harga_total, ""));
	...
	$pdf->Output("SIMDO_RekapInventSKPD.pdf","I");
?>