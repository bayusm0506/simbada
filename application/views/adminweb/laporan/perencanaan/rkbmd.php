<?php
	...
	$this->load->library('perencanaan/PDF_MC_Table_rkbmd');
	
	$pdf=new PDF_MC_Table_rkbmd('L','mm','f4');
	$pdf->Open();
	$pdf->AddPage();
	$pdf->Image(base_url().'asset/img/logo.png',10,5,22,0,'PNG');
	$pdf->SetFont('Arial','B',15);
	$pdf->MultiCell(435,5,"RENCANA KEBUTUHAN BARANG MILIK DAERAH PENGADAAN \n ".NM_PEMDA,0,'C');
	...
	$pdf->SetWidths(array(15,150,40,60,40,40,55,40));
	$pdf->Rowheader();
	...
	// 		$pdf->Row(array($no,  $row->Nm_Aset6, $row->Spesifikasi, rp($row->Jumlah), rp($row->Harga),  rp($row->Jumlah*$row->Harga), $rek, $row->Keterangan));
	// 		$no++;
	// 	}
	// }else{
	// 	$pdf->nihil();
