...
<script>
	...
	if(confirm("Yakin data yang dipilih akan dihapus dalam usulan penghapusan ??")){	
		...
		$("input.checkbox1:checked").each(function(){
			...
			$.ajax({
					type: "POST",
					url: "<?php echo base_url(); ?>penghapusan/ajax_hapus_c",
					data:kode,
					cache: false,
...
$(".batal_usulan").click(function(event) {
	...
	if(confirm("Yakin data ini akan dihapus dalam usulan penghapusan ??"))
	{
		...
		$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>penghapusan/hapus_usul_c",
				data:kode,
			 	cache: false,
			 	success: function(html){
			 		$("#listhapus_"+no).fadeTo(300,0, function() {
...
<a class="a-link" href="'.site_url('penghapusan/rincian_kibc/'.encrypt_url($sk_tmp)).'"> Daftar Penghapusan </a> <span class="pull-right">SK PENGHAPUSAN : ' . strtoupper($sk_tmp) . '</span></p>': '';?>
