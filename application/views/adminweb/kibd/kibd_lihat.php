...
$(document).ready(function(){
		...
		$('a.delete').on('click',function(e){
			...
			if(confirm("Yakin data ini mau di Hapus ??"))
			{
				...
				$.ajax({
						type: "POST",
						url: "<?php echo base_url(); ?>kibd/hapus_gambar",
						data:kode,
						cache: false,
...
<div class="row">
                    ...
                    <div class="panel-header">
                    	...
                    	<span class="pull-right">
                    		<?php 
                    			...
                    			echo '<i class="icon-print"></i> <a class="cetak" href="'.base_url().'cetak/kibd_history/'.encrypt_url($url).'">Cetak Riwayat</a>';
                    		?>
                    	</span>
                    </div>
		  ...
		  echo $tgl; ?></span></td>
                            ...
                            <tr>
                                <td class="description">Konstruksi</td>
                                <td class="value"><span><strong><?php echo $default['Konstruksi']; ?></strong></span></td>
                            </tr>
                            
                            <tr>
                                <td class="description">Panjang</td>
                                <td class="value"><span><strong><?php echo $default['Panjang']; ?></strong></span></td>
                            </tr>
                            <tr>
                                <td class="description">Lebar</td>
                                <td class="value"><span><strong><?php echo $default['Lebar']; ?></strong></span></td>
                            </tr>
                            <tr>
                              <td class="description">Luas</td>
                              <td class="value"><?php echo $default['Luas']; ?></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="value"><span><strong><?php  $tgl 		= date('Y-m-d', strtotime($default['Dokumen_Tanggal']));
		  echo $tgl; ?></strong></span></td>
                            </tr>
                             <tr>
                             ...
                             <tr>
                               ...
                               <td class="value"><?php echo $default['Kd_Tanah1'].'.'.$default['Kd_Tanah2'].'.'.$default['Kd_Tanah3'].'.'.$default['Kd_Tanah4'].'.'.$default['Kd_Tanah5'].'.'.$default['Kode_Tanah']; ?></td>
                             </tr>
                             <tr>
                ...
                <div class="panel">
                    <div class="panel-header">
                    	...
                    	<span class="pull-right">
                    		<?php 
                    			$url = $default['Kd_Prov']."/".$default['Kd_Kab_Kota']."/".$default['Kd_Bidang']."/".$default['Kd_Unit']."/".$default['Kd_Sub']."/".$default['Kd_UPB']."/".$default['Kd_Aset1']."/".$default['Kd_Aset2']."/".$default['Kd_Aset3']."/".$default['Kd_Aset4']."/".$default['Kd_Aset5']."/".$default['No_Register'];
                    			echo '<i class="icon-plus"></i> <a href="'.base_url().'kibd/addriwayat/'.$url.'">Tambah Riwayat</a>';
                    		?>
                    	</span>
                    </div>
                    <div class="panel-content">
	                    	...
	                    	?>
	                    	<?php if($this->session->userdata('lvl') == '01' || thn($row->Tgl_Dokumen) == '2016'){ ?>
		                    <div class="pull-right">
   		                    			<?php echo '<span>'.anchor('kibd/update_riwayat/'.$row->Kd_Riwayat.'/'.$row->Kd_Id.'/'.$row->Kd_Prov.'/'.$row->Kd_Kab_Kota.'/'.$row->Kd_Bidang.'/'.$row->Kd_Unit.'/'.$row->Kd_Sub.'/'.$row->Kd_UPB.'/'.$row->Kd_Aset1.'/'.$row->Kd_Aset2.'/'.$row->Kd_Aset3.'/'.$row->Kd_Aset4.'/'.$row->Kd_Aset5.'/'.$row->No_Register,"<i class='icon-pencil'></i>Ubah",array('class' => 'update')).'</span>'; ?> | 
...
<?php 
			foreach ($query->result() as $row){
				$gambar = base_url("assets/uploads_kibd/$row->Nama_foto");
	  ...
	  <form action="<?php echo site_url('kibd/upload'); ?>" method="post" enctype="multipart/form-data" id="form-upload">
				<table>
					<thead>
