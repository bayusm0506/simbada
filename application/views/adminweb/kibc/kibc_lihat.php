...
$(document).ready(function(){
		...
		$('a.delete').on('click',function(e){
			...
			if(confirm("Yakin data ini mau di Hapus ??"))
			{
				...
				$.ajax({
						type: "POST",
						url: "<?php echo base_url(); ?>kibc/hapus_gambar",
						data:kode,
						cache: false,
	...
	if(confirm("Yakin data ini mau di Hapus ??"))
	{
		...
		$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>kibc/hapus_riwayat",
				data:kode,
			 	cache: false,
			 	success: function(html){
...
$(function() {
		...
		var page = "<?php echo site_url('kibc/form_edit_spec')?>/"+kd_prov+"/"+kd_kab_kota+"/"+kd_bidang+"/"+kd_unit+"/"+kd_sub+"/"+kd_upb+"/"+kd_aset1+"/"+kd_aset2+"/"+kd_aset3+"/"+kd_aset4+"/"+kd_aset5+"/"+no_register;
			...
			buttons: {
				"Proses": function() {
					...
					var kode = {kd_bidang:kd_bidang,
								...
								no_register:no_register,
								Luas_Lantai:$("#Luas_Lantai").val(),
								Lokasi:$("#Lokasi").val(),
								Bertingkat_Tidak:$("#Bertingkat_Tidak").val(),
								Beton_tidak:$("#Beton_tidak").val(),
								Kd_Data:$("#Kd_Data").val(),
								Keterangan:$("#Keterangan").val()
								};

				  // alert($("#Lokasi").val()); return false;
								
				  $.ajax({
					url : "<?php echo site_url('kibc/proses_edit_spec')?>",
					type : 'POST',
					data : kode,
...
<div class="row">
                    ...
                    <div class="panel-header">
                    	...
                    	<span class="pull-right">
                    		...
                    		<?php 
                    			...
                    			echo '<i class="icon-print"></i> <a class="cetak" href="'.base_url().'cetak/kibc_history/'.encrypt_url($url).'">Cetak Riwayat</a>';
                    		?>
                    	</span>
                    </div>
		  ...
		  echo $tgl; ?></span></td>
                            ...
                            <tr>
                                ...
                                <td class="value"><span><strong><?php echo $default['Bertingkat_Tidak']; ?></strong></span></td>
                            </tr>
                            
                            <tr>
                                <td class="description">Beton/Tidak</td>
                                <td class="value"><span><strong><?php echo $default['Beton_tidak']; ?></strong></span></td>
                            </tr>
                            <tr>
                                <td class="description">Luas Lantai</td>
                                <td class="value"><span><strong><?php echo $default['Luas_Lantai']; ?></strong></span></td>
                            </tr>
                            <tr>
                              <td class="description">Lokasi</td>
                              <td class="value"><?php echo $default['Lokasi']; ?></td>
                            </tr>
                            <tr>
                                <td class="description">Tanggal Dokumen</td>
                                <td class="value"><span><strong><?php echo tgl_dmy($default['Dokumen_Tanggal']); ?></strong></span></td>
                            </tr>
                             <tr>
                                <td class="description">Nomor Dokumen</td>
                                <td class="value"><span><strong><?php echo $default['Dokumen_Nomor']; ?></strong></span></td>
                            </tr>
                             <tr>
                               <td class="description">Kode Tanah</td>
                               <td class="value"><?php echo $default['Kd_Tanah1'].'.'.$default['Kd_Tanah2'].'.'.$default['Kd_Tanah3'].'.
							   '.$default['Kd_Tanah4'].'.'.$default['Kd_Tanah5'].'.'.$default['Kode_Tanah']; ?></td>
                             </tr>
                             <tr>
                                <td class="description">Status Tanah</td>
                                <td class="value"><span><strong><?php echo $default['Status_Tanah']; ?></strong></span></td>
                            </tr>
                             <tr>
                            ...
                            <tr>
                                ...
                                <div class="pull-right">
   		                    			<?php 
		                    			$url = $default['Kd_Prov']."/".$default['Kd_Kab_Kota']."/".$default['Kd_Bidang']."/".$default['Kd_Unit']."/".$default['Kd_Sub']."/".$default['Kd_UPB']."/".$default['Kd_Aset1']."/".$default['Kd_Aset2']."/".$default['Kd_Aset3']."/".$default['Kd_Aset4']."/".$default['Kd_Aset5']."/".$default['No_Register'];
		                    			echo '<i class="icon-pencil"></i> <a href="'.base_url().'kibc/addpeta/'.$url.'">Ubah Peta</a>';
                    				?>
		                    	</div>
                            	...
                            	<?php }else{ ?>
	                    			<?php 
		                    			$url = $default['Kd_Prov']."/".$default['Kd_Kab_Kota']."/".$default['Kd_Bidang']."/".$default['Kd_Unit']."/".$default['Kd_Sub']."/".$default['Kd_UPB']."/".$default['Kd_Aset1']."/".$default['Kd_Aset2']."/".$default['Kd_Aset3']."/".$default['Kd_Aset4']."/".$default['Kd_Aset5']."/".$default['No_Register'];
		                    			echo '<i class="icon-plus"></i> <a href="'.base_url().'kibc/addpeta/'.$url.'">Tambah Peta</a>';
                    				?>
                            	<?php } ?>
                            </tr>
                            </tbody>
                ...
                <div class="panel">
                    <div class="panel-header">
                    	...
                    	<span class="pull-right">
                    		<?php 
                    			$url = $default['Kd_Prov']."/".$default['Kd_Kab_Kota']."/".$default['Kd_Bidang']."/".$default['Kd_Unit']."/".$default['Kd_Sub']."/".$default['Kd_UPB']."/".$default['Kd_Aset1']."/".$default['Kd_Aset2']."/".$default['Kd_Aset3']."/".$default['Kd_Aset4']."/".$default['Kd_Aset5']."/".$default['No_Register'];
                    			echo '<i class="icon-plus"></i> <a href="'.base_url().'kibc/addriwayat/'.$url.'">Tambah Riwayat</a>';
                    		?>
                    	</span>
                    </div>
                    <div class="panel-content">
		                    ...
		                    <div class="pull-right">
   		                    			<?php echo '<span>'.anchor('kibc/update_riwayat/'.$row->Kd_Riwayat.'/'.$row->Kd_Id.'/'.$row->Kd_Prov.'/'.$row->Kd_Kab_Kota.'/'.$row->Kd_Bidang.'/'.$row->Kd_Unit.'/'.$row->Kd_Sub.'/'.$row->Kd_UPB.'/'.$row->Kd_Aset1.'/'.$row->Kd_Aset2.'/'.$row->Kd_Aset3.'/'.$row->Kd_Aset4.'/'.$row->Kd_Aset5.'/'.$row->No_Register,"<i class='icon-pencil'></i>Ubah",array('class' => 'update')).'</span>'; ?> |
...
<?php 
			foreach ($query->result() as $row){
				$gambar = base_url("assets/uploads_kibc/$row->Nama_foto");
	  ...
	  <form action="<?php echo site_url('kibc/upload'); ?>" method="post" enctype="multipart/form-data" id="form-upload">
				<table>
					<thead>
