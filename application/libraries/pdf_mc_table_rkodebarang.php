...
class PDF_MC_Table_rkodebarang extends FPDF
{
...
function RowData($data)
{
    $nb=0;
    for($i=0;$i<count($data);$i++)
        $nb=max($nb,$this->NbLines($this->width[$i],$data[$i]));
    $h=5*$nb;
    $this->CheckPageBreak($h);
    for($i=0;$i<count($data);$i++)
    {
...
function UPBTitle($bidang,$unit,$sub_unit,$upb,$kode_lokasi)
{
		...
		$this->Ln();
		$this->Cell(19,5,'KABUPATEN',0,0,'L');$this->Cell(35,5,'',0,0,'L');$this->Cell(0,5,': '.strtoupper(getInfoPemda('Nm_Pemda')),0,0,'L');
		$this->Ln();
...
function UPBTitle2($bidang,$unit,$sub_unit,$upb,$kode_lokasi)
{
		...
		$this->Cell(35,5,' ',0,0,'L');$this->Cell(0,5,': '.strtoupper(getInfoPemda('Nm_Pemda')),0,0,'L');  
		$this->Cell(35,5,' ',0,0,'L');
		$this->Cell(35,5,' ',0,0,'L');
...
function Rowheader()
{
		...
		$this->SetXY(25,$y2+5);
		$this->MultiCell(50,5,"Kode Barang",0,'C');
		
		
		$this->SetXY(75,$y);
		$this->MultiCell(85,20,"",1,'C');
		$this->SetXY(75,$y2+5);
		$this->MultiCell(85,5,"Nama / Jenis Barang",0,'C');
		...
		$this->MultiCell(50,20,"",1,'C');
		$this->SetXY(190,$y2+5);
		$this->MultiCell(50,5,"Total Nilai",0,'C');
		
		/*
		$this->SetXY(195,$y);
		$this->MultiCell(45,20,"",1,'C');
		$this->SetXY(195,$y2+3);
		$this->MultiCell(45,5,"Jumlah Harga\n(Rupiah)",0,'C');*/
		
		$this->SetXY(240,$y);
		$this->MultiCell(50,20,"Keterangan",1,'C');
}
...
function Header()
{
    ...
    $this->MultiCell(280,5,"REKAPITULASI BARANG MILIK DAERAH MENURUT KODE BARANG \n ".NM_PEMDA,0,'C');
	$this->Ln();
	$this->SetFont('Arial','B','10');
	$this->SetWidths(array(15,50,85,30,50,50));
	$this->RowJudul(array("1", "2", "3", "4","5", "6"));
