...
$(".status_operasional").click(function(event) {
	var no          = $(this).attr("no");
	var kd_bidang   = $(this).attr("Kd_Bidang");
	var kd_unit     = $(this).attr("Kd_Unit");
	var kd_sub      = $(this).attr("Kd_Sub");
	...
	if(confirm("Ubah status operasional ??"))
	{
		...
		$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>kibb/operasional",
				data:kode,
			 	cache: false,
			 	success: function(html){
			 		window.location.reload(true);
...
$(function() {
		...
		var page = "<?php echo site_url('kibb/add_usul_hapus')?>";
		var $dialog = $('<div></div>')
		               // .html('<iframe style="border: 0px; " src="' + page + '" width="100%" height="100%"></iframe>')
   		               .load(page)
		               .dialog({
		    title:"Usul Penghapusan Barang KIB B. Peralatan dan Mesin",
		    modal: true,
		    draggable: false,
		    resizable: false,
		    position: ['center'],
...
$(function() {
		...
		var page = "<?php echo site_url('kibb/add_pindah_skpd')?>";
		var $dialog = $('<div></div>')
		               // .html('<iframe style="border: 0px; " src="' + page + '" width="100%" height="100%"></iframe>')
   		               .load(page)
		               .dialog({
			...
			buttons: {
				"Proses": function() {
				  ...
				  $.ajax({
					url : "<?php echo site_url('kibb/proses_pindah_skpd')?>",
					type : 'POST',
					data : kode,
					success: function(msg){
...
$(function() {
		...
		$(".pindah_upb").live("click",function(){
		var no          = $(this).attr("no");
		var kd_bidang   = $(this).attr("Kd_Bidang");
		var kd_unit     = $(this).attr("Kd_Unit");
		var kd_sub      = $(this).attr("Kd_Sub");
		...
		var page = "<?php echo site_url('kibb/add_pindah_upb')?>";
		var $dialog = $('<div></div>')
		               // .html('<iframe style="border: 0px; " src="' + page + '" width="100%" height="100%"></iframe>')
   		               .load(page)
		               .dialog({
			...
			buttons: {
				...
				// alert($("#idupb").val()); return false;				
				  $.ajax({
					url : "<?php echo site_url('kibb/proses_pindah_upb')?>",
					type : 'POST',
					data : kode,
					success: function(msg){
							// alert('oke '+msg); return false;
							$dialog.dialog('close');
							window.location.reload(true);
...
$(function() {
		...
		$(".pindah_ruang").live("click",function(){
		var no          = $(this).attr("no");
		var kd_bidang   = $(this).attr("Kd_Bidang");
		var kd_unit     = $(this).attr("Kd_Unit");
		var kd_sub      = $(this).attr("Kd_Sub");
		...
		var page = "<?php echo site_url('kibb/add_pindah_ruang')?>";
		var $dialog = $('<div></div>')
		               // .html('<iframe style="border: 0px; " src="' + page + '" width="100%" height="100%"></iframe>')
   		               .load(page)
		               .dialog({
			...
			buttons: {
				"Proses": function() {
					...
					var kode = {
								...
								kd_aset4:kd_aset4,kd_aset5:kd_aset5,
								no_register:no_register,
								kd_ruang:$("#idruang").val()
								};
								
				  $.ajax({
					url : "<?php echo site_url('kibb/proses_pindah_ruang')?>",
					type : 'POST',
					data : kode,
					success: function(msg){
							// alert('pilih bidang terlebih dahulu '+msg); return false;
                ...
                <div class="panel">
             	...
             	<div class="panel-content list-content">
                        ...
                        <ul class="nav nav-tabs">
                            ...
                            <a href="<?php echo ! empty($tab1) ?  $tab1: '#'; ?>" class="tab-page" data-toggle="tab">DATA KIB B. PERALATAN & MESIN
                  ...
                  <table class="table">
                    ...
                    echo "<td>$i</td>
						  ...
						  <td>".ucwords(strtolower(ruang($row->Kd_Bidang,$row->Kd_Unit,$row->Kd_Sub,$row->Kd_UPB,$row->Kd_Ruang)))."</td>
		       	...
		       	echo "<li>".anchor('kibb/lihat/'.$row->Kd_Prov.'/'.$row->Kd_Kab_Kota.'/'.$row->Kd_Bidang.'/'.$row->Kd_Unit.'/'.$row->Kd_Sub.'/'.$row->Kd_UPB.'/'.$row->Kd_Aset1.'/'.$row->Kd_Aset2.'/'.$row->Kd_Aset3.'/'.$row->Kd_Aset4.'/'.$row->Kd_Aset5.'/'.$row->No_Register,"<i class='icon-eye-open'></i>Lihat",array('class' => 'update'))."</li>";
		       	echo "<li>".anchor('kibb/cetak/'.$row->Kd_Prov.'/'.$row->Kd_Kab_Kota.'/'.$row->Kd_Bidang.'/'.$row->Kd_Unit.'/'.$row->Kd_Sub.'/'.$row->Kd_UPB.'/'.$row->Kd_Aset1.'/'.$row->Kd_Aset2.'/'.$row->Kd_Aset3.'/'.$row->Kd_Aset4.'/'.$row->Kd_Aset5.'/'.$row->No_Register,"<i class='icon-print'></i>Cetak Kartu",array('class' => 'update'))."</li>";
			
			if(empty($row->Penghapusan) || ($row->Penghapusan == 3)){

				if($row->Penghapusan <> 3){
					if($this->session->userdata('lvl') == '01' || $this->session->userdata('jabatan_id') == '4'){                                    
						echo "<li>".anchor('kibb/update/'.$row->Kd_Prov.'/'.$row->Kd_Kab_Kota.'/'.$row->Kd_Bidang.'/'.$row->Kd_Unit.'/'.$row->Kd_Sub.'/'.$row->Kd_UPB.'/'.$row->Kd_Aset1.'/'.$row->Kd_Aset2.'/'.$row->Kd_Aset3.'/'.$row->Kd_Aset4.'/'.$row->Kd_Aset5.'/'.$row->No_Register,"<i class='icon-pencil'></i>Ubah",array('class' => 'update'))."</li>";
					}
				...
				if($this->session->userdata('lvl') == '01' || $this->session->userdata('lvl') == '02'){
					echo "<li><a class='pindah_upb pointer' Kd_Prov='$row->Kd_Prov' Kd_Kab_Kota='$row->Kd_Kab_Kota' Kd_Bidang='$row->Kd_Bidang' Kd_Unit='$row->Kd_Unit' Kd_Sub='$row->Kd_Sub' Kd_UPB='$row->Kd_UPB' Kd_Aset1='$row->Kd_Aset1' Kd_Aset2='$row->Kd_Aset2' Kd_Aset3='$row->Kd_Aset3' Kd_Aset4='$row->Kd_Aset4' Kd_Aset5='$row->Kd_Aset5' No_Register='$row->No_Register' no='$i'><i class='icon-refresh'></i>Pindah UPB</a></li>";
				}
				...
				echo "<li><a class='pindah_ruang pointer' Kd_Prov='$row->Kd_Prov' Kd_Kab_Kota='$row->Kd_Kab_Kota' Kd_Bidang='$row->Kd_Bidang' Kd_Unit='$row->Kd_Unit' Kd_Sub='$row->Kd_Sub' Kd_UPB='$row->Kd_UPB' Kd_Aset1='$row->Kd_Aset1' Kd_Aset2='$row->Kd_Aset2' Kd_Aset3='$row->Kd_Aset3' Kd_Aset4='$row->Kd_Aset4' Kd_Aset5='$row->Kd_Aset5' No_Register='$row->No_Register' no='$i'><i class='icon-refresh'></i>Pindah Ruang</a></li>";
				...
				echo '<li>'.anchor(current_url().'#',"<i class='fa fa-exclamation-circle'></i> Ubah Status Operasional",array('class'=> 'status_operasional','Kd_Prov' => $row->Kd_Prov,'Kd_Kab_Kota' => $row->Kd_Kab_Kota,'Kd_Bidang' => $row->Kd_Bidang,'Kd_Unit' => $row->Kd_Unit,'Kd_Sub' => $row->Kd_Sub,'Kd_UPB' => $row->Kd_UPB,'Kd_Aset1' => $row->Kd_Aset1,'Kd_Aset2' => $row->Kd_Aset2,'Kd_Aset3' => $row->Kd_Aset3,'Kd_Aset4' => $row->Kd_Aset4,'Kd_Aset5' => $row->Kd_Aset5,'No_Register' => $row->No_Register,'Kd_KA' => $row->Kd_KA,'no'=>$i)).'</li>';
