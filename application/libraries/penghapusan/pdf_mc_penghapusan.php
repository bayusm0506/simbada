...
class PDF_MC_Penghapusan extends FPDF
{
...
function Rowheader()
{
		...
		$this->SetXY(75,$y);
		$this->MultiCell(120,5,"SPESIFIKASI BARANG",1,'C');
		$this->SetXY(75,$y2+3);
		$this->MultiCell(60,15,"Nama/Jenis Barang",1,'C');
		$this->SetXY(135,$y2+3);
		...
		$this->SetXY(195,$y);
		$this->MultiCell(20,20,"Bahan",1,'C');
		...
		$this->MultiCell(25,20,"",1,'C');
		$this->SetXY(265,$y+2);
		$this->MultiCell(25,4,"Ukuran Barang/ Konstruksi\n(P,SP,D)",0,'C');
		...
		$this->MultiCell(25,20,"",1,'C');
		$this->SetXY(310,$y+4);
		$this->MultiCell(25,4,"Kondisi\n(B, KB, RB)",0,'C');
		
		$this->SetXY(335,$y);
		$this->MultiCell(65,5,"JUMLAH",1,'C');
		$this->SetXY(335,$y2+3);
		$this->MultiCell(25,15,"Barang",1,'C');
		$this->SetXY(360,$y2+3);
		$this->MultiCell(40,15,"Harga",1,'C');
		
		$this->SetXY(400,$y);
		$this->MultiCell(50,20,"Ket.",1,'C');
}
...
function Rowheader2()
{
		...
		$this->SetXY(400,$y);
		$this->MultiCell(50,20,"Nama UPB",1,'C');
}
...
function ttd2($nm_pimpinan,$nip_pimpinan,$jbt_pimpinan,$nm_pengurus,$nip_pengurus,$tanggal)
{
	...
	$this->Cell(90,5,$tanggal,0,'LR', 'C');
	$this->Ln();
	$this->MultiCell(90,5,"a.n. GUBERNUR SUMATERA UTARA \n Plt. SEKRETARIS DAERAH KABUPATEN",0,'C');
	$this->SetXY($r1+350,$y1+5);
	$this->MultiCell(90,5,"KEPALA DINAS PENGELOLAAN KEUANGAN \n DAN ASET DAERAH",0,'C');
	
	$this->Ln(25);
	$this->SetFont('Arial','UB',11);
	$this->Cell(90,5,'Drs. MHD. SYAFEI LUBIS, M.Si',0,'B', 'C');  
	$this->Cell(615,5,'H. SAMAD, SE, MM',0,'B', 'C'); 
	
	$this->Ln();
	$this->SetFont('Arial','',11);
	$this->Cell(90,5,'NIP . 195911091986021002',0,'B', 'C');
	$this->Cell(615,5,'NIP . 195912311988011012',0,'B', 'C');
...
function Header()
{
    ...
    $this->Cell(0,9,'DAFTAR KONDISI ASET',0,0,'C');
	$this->Ln();
	$this->SetFont('Arial','B','10');
