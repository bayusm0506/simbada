...
$(document).ready(function(){
		...
		$(".hapus").click(function(event) {
			...
			if(confirm("Yakin data ini mau di Hapus ??"))
			{
				...
				$.ajax({
						type: "POST",
						url: "<?php echo base_url(); ?>kibd/ajax_hapus",
						data:kode,
					 	cache: false,
		...
		$(".aktifkan_semua").click(function(event) {
			if(confirm("Yakin data ini sudah benar dan akan diaktifkan ??")){
				...
				$("input.checkbox1:checked").each(function(){
					...
					$.ajax({
							type: "POST",
							url: "<?php echo base_url(); ?>kibd/aktifkan",
							data:kode,
							cache: false,
							success: function(html){
								$("#listhapus_"+no).fadeTo(300,0, function() {
		...
		$(".usul_guna").click(function(event) {
			...
			if(confirm("Data ini akan diusulkan untuk penetapan status penggunaan barang ??"))
			{
				...
				$.ajax({
						type: "POST",
						url: "<?php echo base_url(); ?>kibd/usul_guna",
						data:kode,
					 	cache: false,
					 	success: function(html){
		...
		$(".non_operasional").click(function(event) {
				...
				if(confirm("Ubah status operasional ??"))
				{
					...
					$.ajax({
							type: "POST",
							url: "<?php echo base_url(); ?>kibd/operasional",
							data:kode,
						 	cache: false,
						 	success: function(html){
...
$(function() {
		...
		var page = "<?php echo site_url('kibd/add_usul_hapus')?>";
		var $dialog = $('<div></div>')
		               // .html('<iframe style="border: 0px; " src="' + page + '" width="100%" height="100%"></iframe>')
   		               .load(page)
		               .dialog({
		    title:"Usul Penghapusan Barang KIB D. Jalan, Irigasi dan Jaringan",
		    modal: true,
		    draggable: false,
		    resizable: false,
		    position: ['center'],
			...
			buttons: {
				"Proses": function() {
				  ...
				  $.ajax({
					url : "<?php echo site_url('kibd/proses_usul_hapus')?>",
					type : 'POST',
					data : kode,
					success: function(msg){
...
$(function() {
		...
		var page = "<?php echo site_url('kibd/add_pindah_skpd')?>";
		var $dialog = $('<div></div>')
		               // .html('<iframe style="border: 0px; " src="' + page + '" width="100%" height="100%"></iframe>')
   		               .load(page)
		               .dialog({
			...
			buttons: {
				"Proses": function() {
				  ...
				  $.ajax({
					url : "<?php echo site_url('kibd/proses_pindah_skpd')?>",
					type : 'POST',
					data : kode,
					success: function(msg){
...
<div class="row">
            <div class="span12">
                ...
                <div class="panel">
                    <div class="panel-header"><i class="icon-group"></i><?php echo ! empty($header) ?  $header: 'D. JALAN,IRIGASI & JARINGAN'; ?>
             	...
             	<div class="panel-content list-content">
                        ...
                        <ul class="nav nav-tabs">
                            ...
                            <a href="<?php echo ! empty($tab1) ?  $tab1: '#'; ?>" class="tab-page" data-toggle="tab">DATA KIB D. JALAN,IRIGASI & JARINGAN
                       ...
                       <div class="tab-content">
                            <div class="tab-pane list-pane active" id="tab1">
                                ...
                                <div class="pull-right">
                                    <form class="form-inline" method="POST" id="form_view" action=<?php echo base_URL(); ?>kibd/set/>
                	...
                	<div class="list-actions">
                            <div class="btn-toolbar">
                                ...
                                <div class="btn-group">
                                    <a href="<?php echo base_url() ?>kibd/add">
                          ...
                          <td width='20%'>$row->Keterangan</td>
                            <td>
                                            <div class='btn-group'>
                                                ...
                                                <ul class='dropdown-menu'><li>".anchor('kibd/lihat/'
