...
<script>
	...
	$(".batal_hapus").click(function(event) {
	var no          = $(this).attr("no");
	...
	if(confirm("Yakin data ini akan dihapus dalam daftar penghapusan ??"))
	{
		$( "#loading" ).empty().append("<img src='<?php echo base_url();?>media/images/ui-anim_basic_16x16.gif' /> tunggu sebentar...");
		$.ajax({
				type: "POST",
				url: "<?php echo base_url() ?>penghapusan/proses_usul_b",
				data:kode,
			 	cache: false,
	...
	if(confirm("Yakin data yang dipilih akan dihapus dalam usulan penghapusan ??")){	
		...
		$("input.checkbox1:checked").each(function(){
			...
			$.ajax({
					type: "POST",
					url: "<?php echo base_url(); ?>penghapusan/batal_hapus_b",
					data:kode,
					cache: false,
					success: function(html){
						$("#listhapus_"+no).fadeTo(300,0, function() {
                       ...
                       <div class="tab-content">
                            <div class="tab-pane list-pane active" id="tab1">
                                ...
                                <div class="pull-right">
                                    <form class="form-inline" method="POST" id="form_view" action="<?php echo $form_action; ?>">
                	...
                	<div class="list-actions">
                            <div class="btn-toolbar">
                                ...
                                <div class="btn-group">
                                    ...
                                    <button class="btn btn-success"><i class="icon-plus-sign"></i> Tambah data dari usulan penghapusan</button>
                                    </a>
                                </div>
                      ...
                      echo "<tr id='listhapus_$i'>
                          ...
                          <td>
                                            <div class='btn-group'>
                                                ...
                                                <ul class='dropdown-menu'>
                                                	<li>".anchor(current_url().'#',"<i class='icon-retweet'></i>Batalkan Penghapusan",array('class'=> 'batal_hapus','Kd_Prov' => $row->Kd_Prov,'Kd_Kab_Kota' => $row->Kd_Kab_Kota,'Kd_Bidang' => $row->Kd_Bidang,'Kd_Unit' => $row->Kd_Unit,'Kd_Sub' => $row->Kd_Sub,'Kd_UPB' => $row->Kd_UPB,'Kd_Aset1' => $row->Kd_Aset1,'Kd_Aset2' => $row->Kd_Aset2,'Kd_Aset3' => $row->Kd_Aset3,'Kd_Aset4' => $row->Kd_Aset4,'Kd_Aset5' => $row->Kd_Aset5,'No_Register' => $row->No_Register,'Kd_Id' => $row->Kd_Id,'Status' => 1,'no'=>$i))."</li>
