...
class Rkbu extends CI_Controller {
	...
	function __construct(){
		parent::__construct();
		$this->auth->restrict();
		// if(!$this->general->privilege_check('rkbu',VIEW))
		//     $this->general->no_access();

		$this->auth->clean_session('rkbu');
	...
	{
		$data['form_cari']	= site_url('rkbu/cari');
		$data['link_kib']	= site_url('rkbu/listupb');
		...
		$data['link'] = array('link_add' => anchor('rkbu/add/','tambah data', array('class' => ADD)));
	...
	function listupb($bidang,$unit,$sub)
	{
		...
		$data['link_kib']	= site_url('rkbu/upb');
	...
	/**
	 * Tampilkan semua data RKBU kib a
	 */
	function upb($kd_bidang='',$kd_unit='',$kd_sub='',$kd_upb='')
	{
		...
		$get               = $this->Rkbu_model->get_page($this->limit, $offset, $where, $like2);
		$data['query']     = $get['data'];
		$num_rows          = $get['total'];
		...
		{
			$data['message'] = 'Tidak ditemukan data RKBU!';
		}
		$this->template->load('template','adminweb/rkbu/rkbu',$data);
	}
	...
	/**
	 * Pindah ke halaman update rkbu
	 */
	function lihat($kontrak)
	{
		$data['title'] 			= $this->title;
		$data['h2_title'] 		= 'rkbu > Rincian';
		$data['form_action']	= site_url('rkbu/update_process');
		$data['link'] 			= array('link_back' => anchor('Penghapusan','kembali', array('class' => 'back'))
										);
		$data['header'] 		= $this->title;
		$data['judul'] 			= "Rincian rkbu";
		$kon = $kontrak.'==';
		...
		$rkbu 	= $this->Rkbu_model->get_rkbu_by_id($n_kontrak)->row();
		$num_rows 	= $this->Rkbu_model->get_rincian($n_kontrak)->num_rows();

		$nmupb				= "";
		$data['query'] 		= $this->Rkbu_model->get_rincian($n_kontrak);
		$num_rows 			= $num_rows ;
		$total 				= 55;
		$nilai_spk			= number_format($rkbu->Nilai,0,",",".");

		$data['header'] 	= ' No Kontrak : '.$rkbu->No_Kontrak.' | Tanggal : '.tgl_indo($rkbu->Tgl_Kontrak).' | Nilai SPK : Rp. '.$nilai_spk;
		$data['No_Kontrak'] = $rkbu->No_Kontrak;
		...
		{
			$data['message'] = 'Tidak ditemukan rincian data RKBU!';
		}

		$this->template->load('template','adminweb/rkbu/rkbu_lihat',$data);
	}
	...
	function export(){
		...
		if (empty($tanggal1) AND empty($tanggal2) AND empty($q) AND empty($s)){
			...
			$like2 	= "";
			$judul 	= "Semua Data usulan rkbu KIB A. Tanah";
		}elseif (empty($tanggal1) AND empty($tanggal2) AND !empty($q) AND !empty($s)){
			$like2 	= "AND $q LIKE '%$s%'";
			$judul 	= "Pencarian dengan ".cari($q)." $s";
		}else{
		...
		$arg = explode('/', $curl);
		if ($arg[5] == 'skpd' ) {
			$d['data_view'] = $this->db->query("select * from Tb_rkbu inner join Ref_Rek_Aset5 on
Tb_rkbu.Kd_Aset1=Ref_Rek_Aset5.Kd_Aset1 AND
Tb_rkbu.Kd_Aset2=Ref_Rek_Aset5.Kd_Aset2 AND
Tb_rkbu.Kd_Aset3=Ref_Rek_Aset5.Kd_Aset3 AND
Tb_rkbu.Kd_Aset4=Ref_Rek_Aset5.Kd_Aset4 AND
Tb_rkbu.Kd_Aset5=Ref_Rek_Aset5.Kd_Aset5 WHERE $where2 $like2");
		}elseif ($arg[5] == 'upb' ) {
			$d['data_view'] = $this->db->query("select * from Tb_rkbu inner join Ref_Rek_Aset5 on
Tb_rkbu.Kd_Aset1=Ref_Rek_Aset5.Kd_Aset1 AND
Tb_rkbu.Kd_Aset2=Ref_Rek_Aset5.Kd_Aset2 AND
Tb_rkbu.Kd_Aset3=Ref_Rek_Aset5.Kd_Aset3 AND
Tb_rkbu.Kd_Aset4=Ref_Rek_Aset5.Kd_Aset4 AND
Tb_rkbu.Kd_Aset5=Ref_Rek_Aset5.Kd_Aset5 WHERE $where $like2 AND aktif <> NULL");
		}else{
			$d['data_view'] = $this->db->query("select * from Tb_rkbu inner join Ref_Rek_Aset5 on
Tb_rkbu.Kd_Aset1=Ref_Rek_Aset5.Kd_Aset1 AND
Tb_rkbu.Kd_Aset2=Ref_Rek_Aset5.Kd_Aset2 AND
...
Tb_rkbu.Kd_Aset5=Ref_Rek_Aset5.Kd_Aset5 WHERE $where $like2 AND aktif = NULL");
		...
		$this->load->view('adminweb/rkbu/export',$d);
	...
	public function upload()
	{
		if(isset($_FILES['image']))
		{
			...
			{
				$config['upload_path'] = './assets/uploads_rkbu';
				...
				if($this->multi_upload->do_upload($data2[$no]))
				{
					...
					$foto = array(
						...
						$this->Rkbu_model->insert($foto);

					echo img(array(
						'src'=>base_url("assets/uploads_rkbu/$image_data[file_name]"),
						'width'=>179,
    ...
    $row = $this->db->get_where('Ta_FotoA',array('Kd_Prov' => $kd_prov,'Kd_Kab_Kota' => $kd_kab_kota,'Kd_Bidang' => $kd_bidang,'Kd_Unit' => $kd_unit,
     ...
     unlink("./assets/uploads_rkbu/$row->Nama_foto");
	...
	{
		if(!$this->general->privilege_check('rkbu',REMOVE))
		    $this->general->no_access();

		$this->Rkbu_model->hapus($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register);
	...
	function ajax_hapus(){
		...
		$this->Rkbu_model->hapus($tahun,$kd_prov,$kd_kab_kota,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1x,$kd_aset2x,$kd_aset3x,$kd_aset4x,$kd_aset5x,$kd_aset6x,$no_id);
	}
	...
	function hapus_rincian(){
			...
			$No_ID = $this->input->post('No_ID');
			$this->Rkbu_model->hapus_rincian($No_Kontrak,$No_ID);
	}
	...
	function aktifkan(){
		...
		$this->Rkbu_model->sm_update($kd_prov,$kd_kab_kota,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,
	...
	function add()
	{
		// if(!$this->general->privilege_check('rkbu',ADD))
		//     $this->general->no_access();
		...
		$data['title']          = $this->title;
		$data['h2_title']       = 'RKBU > Tambah Data';
		$data['form_action']    = site_url('rkbu/add_process');
		$data['link']           = array('link_back' => anchor('rkbu','kembali', array('class' => 'back'))
		);
		
		$data['option_program'] = $this->Rkbu_model->ProgramList($kd_bidang,$kd_unit,$kd_sub);
		
		$nmupb                  = $this->Ref_upb_model->nama_upb($kd_bidang,$kd_unit,$kd_sub,$kd_upb);
		$data['header']         = $nmupb;

		$this->template->load('template','adminweb/rkbu/rkbu_addform',$data);
	}

	/**
	 * Proses tambah data RKBU
	 */
	function add_process()
	{
			...
			// print_r($data); exit();
			$sql = $this->Rkbu_model->save($data);

			if ($sql){
				$this->session->set_flashdata('message', 'Satu data RKBU berhasil ditambah !');
				redirect('rkbu/upb/'.$this->session->userdata('addKd_Bidang').'/'.$this->session->userdata('addKd_Unit').'/'
	...
	function update($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$kd_aset6,$no_id)
	{
		...
		// if(!$this->general->privilege_check('rkbu',EDIT))
		//     $this->general->no_access();

		$data['title'] 			= $this->title;
		$data['h2_title'] 		= 'RKBU > Update';
		$data['form_action']	= site_url('rkbu/update_process');
		$data['link'] 			= array('link_back' => anchor('rkbu','kembali', array('class' => 'back'))
										);
		$data['header'] 		= $this->title;

		$jumlah = $this->Rkbu_model->get_rkbu_by_id($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,
												  $kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$kd_aset6,$no_id)->num_rows();


		if ($jumlah > 0){
			$get = $this->Rkbu_model->get_rkbu_by_id($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,
												  $kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$kd_aset6,$no_id)->row_array();

			
			$data['option_program']  = $this->Rkbu_model->ProgramList($kd_bidang,$kd_unit,$kd_sub);
			$data['option_kegiatan'] = $this->Rkbu_model->KegiatanList($kd_bidang,$kd_unit,$kd_sub,$get['Kd_Prog']);
			...
			$this->template->load('template','adminweb/rkbu/rkbu_updateform',array_merge($get,$data) );
		}else{
			redirect('rkbu');
		}
	}
	...
	/**
	 * Proses update data RKBU
	 */
	function update_process()
	{
			$data = $this->input->post(NULL, TRUE);
			
			$sql = $this->Rkbu_model->update($data);

			if ($sql){
				$this->session->set_flashdata('message', 'Satu data RKBU berhasil diubah !');
				redirect('rkbu/upb/'.$this->session->userdata('addKd_Bidang').'/'.$this->session->userdata('addKd_Unit').'/'
										.$this->session->userdata('addKd_Sub').'/'.$this->session->userdata('addKd_UPB'));
			}else{
				$this->session->set_flashdata('message', 'Satu data RKBU berhasil diubah !');
	...
	function json(){
        ...
        {
            $this->load->view('rkbu/index',$data);
        }
	}
	...
	function no_id(){
			...
			$num_rows = $this->Rkbu_model->get_last_noid($kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1x,$kd_aset2x,$kd_aset3x,$kd_aset4x,$kd_aset5x,$kd_aset6x);
	...
	function cek_register(){
			...
			$num_rows = $this->Rkbu_model->cek_register($kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1x,$kd_aset2x,$kd_aset3x,$kd_aset4x,$kd_aset5x,$no_registerx)->num_rows();
			echo $num_rows;
			}
	...
	{
		if ($this->Rkbu_model->no_register($no_register) == TRUE)
		{
			$this->form_validation->set_message('valid_id', "rkbu dengan Kode $id_Penghapusan sudah terdaftar");
			return FALSE;
		}
		else
		{
	...
	/**
	 * Cek apakah $id_Penghapusan valid, agar tidak ganda. Hanya untuk proses update data RKBU
	 */
	function cek_register2()
	{
		...
		{
if($this->Rkbu_model->valid_no_register($kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register) === TRUE)
			{
				$this->form_validation->set_message('valid_no_register2', "rkbu dengan kode $new_id sudah terdaftar");
				return FALSE;
			}
			else
			{
	...
	function posting($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_id)
	{
		...
		$get = $this->Rkbu_model->get_rkbu_by_id($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_id)->row_array();
		if(!$get)
	    ...
	    // 	redirect('rkbu/upb/'.$this->session->userdata('addKd_Bidang').'/'.$this->session->userdata('addKd_Unit').'/'.$this->session->userdata('addKd_Sub').'/'.$this->session->userdata('addKd_UPB'));
	    // }

		$data['title'] 			= "Posting Data KIB";
		$data['h2_title'] 		= 'rkbu > POSTING DATA KIB';
		...
		if($get['Kd_Aset1'] == 1){
			$view = "adminweb/rkbu/posting_kiba";
		}elseif($get['Kd_Aset1'] == 2){
			$view = "adminweb/rkbu/posting_kibb";
		}elseif($get['Kd_Aset1'] == 3){
			$view = "adminweb/rkbu/posting_kibc";
		}elseif($get['Kd_Aset1'] == 4){
			$view = "adminweb/rkbu/posting_kibd";
		}elseif($get['Kd_Aset1'] == 5){
			$view = "adminweb/rkbu/posting_kibe";
		}elseif($get['Kd_Aset1'] == 6){
			$view = "adminweb/rkbu/posting_kibf";
		}elseif($get['Kd_Aset1'] == 7){
			$view = "adminweb/rkbu/posting_kibl";
		}else{
			show_error("Tidak ada data!");
		}
...
/* End of file rkbu.php */
/* Location: ./system/application/controllers/rkbu.php */