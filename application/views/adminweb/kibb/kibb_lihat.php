...
$(document).ready(function(){
		...
		$('a.delete').on('click',function(e){
			...
			if(confirm("Yakin data ini mau di Hapus ??"))
			{
				...
				$.ajax({
						type: "POST",
						url: "<?php echo base_url(); ?>kibb/hapus_gambar",
						data:kode,
						cache: false,
	...
	if(confirm("Yakin data ini mau di Hapus ??"))
	{
		...
		$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>kibb/hapus_riwayat",
				data:kode,
			 	cache: false,
			 	success: function(html){
...
$(function() {
		...
		var page = "<?php echo site_url('kibb/form_edit_spec')?>/"+kd_prov+"/"+kd_kab_kota+"/"+kd_bidang+"/"+kd_unit+"/"+kd_sub+"/"+kd_upb+"/"+kd_aset1+"/"+kd_aset2+"/"+kd_aset3+"/"+kd_aset4+"/"+kd_aset5+"/"+no_register;
			...
			buttons: {
				"Proses": function() {
					...
					var kode = {kd_bidang:kd_bidang,
								...
								CC:$("#CC").val(),
								Bahan:$("#Bahan").val(),
								Nomor_Pabrik:$("#Nomor_Pabrik").val(),
								Jumlah_Roda:$("#Jumlah_Roda").val(),
								Nomor_Rangka:$("#Nomor_Rangka").val(),
								Nomor_Mesin:$("#Nomor_Mesin").val(),
								Nomor_BPKB:$("#Nomor_BPKB").val(),
								Nomor_Polisi:$("#Nomor_Polisi").val(),
								Pemakai:$("#Pemakai").val(),
								Keterangan:$("#Keterangan").val()
								};

								// alert($("#Kd_Ruang").val()); return false;
								
				  $.ajax({
					url : "<?php echo site_url('kibb/proses_edit_spec')?>",
					type : 'POST',
					data : kode,
...
<p class="alert alert-info"><a href="<?php echo $this->session->userdata('curl'); ?>"><i class="fa fa-reply-all"></i> Kembali Ke Data</a></p>
...
<div class="row">
                    ...
                    <div class="panel-header">
                    	...
                    	<span class="pull-right">
                    		...
                    		<?php 
                    			...
                    			echo '<i class="icon-print"></i> <a class="cetak" href="'.base_url().'cetak/history/'.encrypt_url($url).'">Cetak Riwayat</a>';
                    		?>
                    	</span>
                    </div>
    ...
    <td class="value"><?php echo ucwords(strtolower(ruang($default['Kd_Bidang'],$default['Kd_Unit'],$default['Kd_Sub'],$default['Kd_UPB'],$default['Kd_Ruang']))); ?></td>
                            </tr>
                            <tr>
		  ...
		  echo $tgl; ?></span></td>
                            ...
                            <tr>
                                <td class="description">Merk</td>
                                <td class="value"><span><strong><?php echo $default['Merk']; ?></strong></span></td>
                            </tr>
                            <tr>
                              <td class="description">Type</td>
                              <td class="value"><strong><?php echo $default['Type']; ?></strong></td>
                            </tr>
                            
                            <tr>
                                <td class="description">Ukuran</td>
                                <td class="value"><span><strong><?php echo $default['CC']; ?></strong></span></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td class="description">No Pabrik</td>
                                <td class="value"><span><strong><?php echo $default['Nomor_Pabrik']; ?></strong></span></td>
                            </tr>
                            <?php if($default['Kd_Aset2']==2 AND $default['Kd_Aset2']==3){ ?>
                            <tr>
                                <td class="description">No Rangka</td>
                                <td class="value"><span><strong><?php echo $default['Nomor_Rangka']; ?></strong></span></td>
                            </tr>
                            <tr>
                                <td class="description">No Mesin</td>
                                <td class="value"><span><strong><?php echo $default['Nomor_Mesin']; ?></strong></span></td>
                            </tr>
                            <tr>
                                <td class="description">No Polisi</td>
                                <td class="value"><span><strong><?php echo $default['Nomor_Polisi']; ?></strong></span></td>
                            </tr>
                             <tr>
                                <td class="description">No BPKB</td>
                                <td class="value"><span><strong><?php echo $default['Nomor_BPKB']; ?></strong></span></td>
                            </tr>
                            <tr>
                                <td class="description">Nama Pemakai</td>
                                <td class="value"><span><strong><?php echo $default['Pemakai']; ?></strong></span></td>
                            </tr>
                             <?php } ?>
                            <tr>
							...
							<tr>
                                ...
                                <td class="value"><span><strong>Rp. <?php echo rp($default['Harga'])." >> ".rp($default['LastHarga']); ?></strong></span></td>
                            </tr>
							<tr>
                ...
                <div class="panel">
                    <div class="panel-header">
                    	...
                    	<span class="pull-right">
                    		<?php 
                    			$url = $default['Kd_Prov']."/".$default['Kd_Kab_Kota']."/".$default['Kd_Bidang']."/".$default['Kd_Unit']."/".$default['Kd_Sub']."/".$default['Kd_UPB']."/".$default['Kd_Aset1']."/".$default['Kd_Aset2']."/".$default['Kd_Aset3']."/".$default['Kd_Aset4']."/".$default['Kd_Aset5']."/".$default['No_Register'];
                    			echo '<i class="icon-plus"></i> <a href="'.base_url().'kibb/addriwayat/'.$url.'">Tambah Riwayat</a>';
                    		?>
                    	</span>
                    </div>
                    <div class="panel-content">
		                    ...
		                    <div class="pull-right">
		                    		<?php echo '<span>'.anchor('kibb/update_riwayat/'.$row->Kd_Riwayat.'/'.$row->Kd_Id.'/'.$row->Kd_Prov.'/'.$row->Kd_Kab_Kota.'/'.$row->Kd_Bidang.'/'.$row->Kd_Unit.'/'.$row->Kd_Sub.'/'.$row->Kd_UPB.'/'.$row->Kd_Aset1.'/'.$row->Kd_Aset2.'/'.$row->Kd_Aset3.'/'.$row->Kd_Aset4.'/'.$row->Kd_Aset5.'/'.$row->No_Register,"<i class='icon-pencil'></i>Ubah",array('class' => 'update')).'</span>'; ?> |
...
<?php 
			foreach ($query->result() as $row){
				$gambar = base_url("assets/uploads_kibb/$row->Nama_foto");
	  ...
	  <form action="<?php echo site_url('kibb/upload'); ?>" method="post" enctype="multipart/form-data" id="form-upload">
				<table>
					<thead>
