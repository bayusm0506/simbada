             	...
             	<div class="panel-content list-content">
                	...
                	<div class="list-actions">
                            <div class="btn-toolbar">
                                ...
                                <div class="btn-group">
                                <?php
									$kd_bidang =  $this->uri->segment(3);
									$kd_unit   =  $this->uri->segment(4);
									$kd_sub    =  $this->uri->segment(5);
									$kd_upb    =  $this->uri->segment(6);
									$url_upb   = $kd_bidang.'/'.$kd_unit.'/'.$kd_sub.'/'.$kd_upb;
                                ?>
                                    <a href="<?php echo base_url() ?>rkbu/add/<?php echo $url_upb ?>">
                                    <button class="btn btn-success"><i class="icon-plus-sign"></i> Tambah Data</button>
                                    </a>
                                    <a href="#" style="padding-left: 5px;">
                                    <button class="btn btn-success"><i class="icon-download"></i> Import Renja</button>
                                    </a>
                                </div>
                          ...
                          <td>$row->Keterangan</td>
	                        <td>
	                            <div class='btn-group'>
	                                ...
	                                <ul class='dropdown-menu'>
	                                	<li>".anchor(current_url().'#',"<i class='icon-trash'></i>Hapus",array('class'=> 'hapus','Tahun' => $row->Tahun,'Kd_Prov' => $row->Kd_Prov,'Kd_Kab_Kota' => $row->Kd_Kab_Kota,'Kd_Bidang' => $row->Kd_Bidang,'Kd_Unit' => $row->Kd_Unit,'Kd_Sub' => $row->Kd_Sub,'Kd_UPB' => $row->Kd_UPB,'Kd_Aset1' => $row->Kd_Aset1,'Kd_Aset2' => $row->Kd_Aset2,'Kd_Aset3' => $row->Kd_Aset3,'Kd_Aset4' => $row->Kd_Aset4,'Kd_Aset5' => $row->Kd_Aset5,'Kd_Aset6' => $row->Kd_Aset6,'No_ID' => $row->No_ID,'no'=>$i))."</li>
	                                	<li>".anchor('rkbu/update/'.$row->Kd_Prov.'/'.$row->Kd_Kab_Kota.'/'.$row->Kd_Bidang.'/'.$row->Kd_Unit.'/'.$row->Kd_Sub.'/'.$row->Kd_UPB.'/'.$row->Kd_Aset1.'/'.$row->Kd_Aset2.'/'.$row->Kd_Aset3.'/'.$row->Kd_Aset4.'/'.$row->Kd_Aset5.'/'.$row->Kd_Aset6.'/'.$row->No_ID,"<i class='icon-pencil'></i>Ubah",array('class' => 'update'))."</li>
									</ul>
	                            </div>
	                        </td>
                      </tr>";
