	...
	$(document).ready(function(){
		...
		$('a.delete').on('click',function(e){
			...
			if(confirm("Yakin data ini mau di Hapus ??"))
			{
				...
				$.ajax({
						type: "POST",
						url: "<?php echo base_url(); ?>lainnya/hapus_gambar",
						data:kode,
						cache: false,
...
<script type="text/javascript">
	...
	if(confirm("Yakin data ini mau di Hapus ??"))
	{
			...
			$.ajax({
					type: "POST",
					url: "<?php echo base_url(); ?>lainnya/hapus_riwayat",
					data:kode,
				 	cache: false,
				 	success: function(html){
                    ...
                    <div class="panel-header">
                    	...
                    	<span class="pull-right">
                    		<?php 
                    			...
                    			echo '<i class="icon-print"></i> <a class="cetak" href="'.base_url().'cetak/lainnya_history/'.encrypt_url($url).'">Cetak Riwayat</a>';
                    		?>
                    	</span>
                    </div>
                ...
                <div class="panel">
                    <div class="panel-header">
                    	...
                    	<span class="pull-right">
                    		<?php 
                    			$url = $default['Kd_Prov']."/".$default['Kd_Kab_Kota']."/".$default['Kd_Bidang']."/".$default['Kd_Unit']."/".$default['Kd_Sub']."/".$default['Kd_UPB']."/".$default['Kd_Aset1']."/".$default['Kd_Aset2']."/".$default['Kd_Aset3']."/".$default['Kd_Aset4']."/".$default['Kd_Aset5']."/".$default['No_Register'];
                    			echo '<i class="icon-plus"></i> <a href="'.base_url().'lainnya/addriwayat/'.$url.'">Tambah Riwayat</a>';
                    		?>
                    	</span>
                    </div>
                    <div class="panel-content">
						...
						?>
		                    ...
		                    <div class="pull-right">
		                    		<?php echo '<span>'.anchor('lainnya/update_riwayat/'.$row->Kd_Riwayat.'/'.$row->Kd_Id.'/'.$row->Kd_Prov.'/'.$row->Kd_Kab_Kota.'/'.$row->Kd_Bidang.'/'.$row->Kd_Unit.'/'.$row->Kd_Sub.'/'.$row->Kd_UPB.'/'.$row->Kd_Aset1.'/'.$row->Kd_Aset2.'/'.$row->Kd_Aset3.'/'.$row->Kd_Aset4.'/'.$row->Kd_Aset5.'/'.$row->No_Register,"<i class='icon-pencil'></i>Ubah",array('class' => 'update')).'</span>'; ?> |
...
<?php 
			foreach ($query->result() as $row){
				$gambar = base_url("assets/uploads_lainnya/$row->Nama_foto");
	  ...
	  <form action="<?php echo site_url('lainnya/upload'); ?>" method="post" enctype="multipart/form-data" id="form-upload">
				<table>
					<thead>
