...
$( "#search" ).change(function() {
	var t = $('#search').val();
	 if (t != "all" && t != ""){    		
										...
										$.ajax({
												type: "POST",
												url : "<?php echo site_url('kibe/select_q')?>",
												data: "q="+t,
...
<script>
	$(function() {
		...
		kd_upb: $("#kd_upb").val(),
				  ...
				  $.ajax({
					url : "<?php echo site_url('kibe/pindahskpd')?>",
					type : 'POST',
					data : kode,
					success: function(msg){
...
<script>
	...
	if(confirm("Yakin data ini mau di Hapus ??"))
	{
		...
		$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>kibe/ajax_hapus",
				data:kode,
			 	cache: false,
...
$(".aktifkan_semua").click(function(event) {
	if(confirm("Yakin data ini sudah benar dan akan diaktifkan ??")){
		...
		$("input.checkbox1:checked").each(function(){
			...
			$.ajax({
					type: "POST",
					url: "<?php echo base_url(); ?>kibe/aktifkan",
					data:kode,
					cache: false,
					success: function(html){
						$("#listhapus_"+no).fadeTo(300,0, function() {
...
<!-- start usul guna  data ->
	...
	if(confirm("Data ini akan diusulkan untuk penetapan status penggunaan barang ??"))
	{
		...
		$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>kibe/usul_guna",
				data:kode,
			 	cache: false,
			 	success: function(html){
...
$(".non_operasional").click(function(event) {
		...
		if(confirm("Ubah status operasional ??"))
		{
			...
			$.ajax({
					type: "POST",
					url: "<?php echo base_url(); ?>kibe/operasional",
					data:kode,
				 	cache: false,
				 	success: function(html){
...
$(function() {
		...
		var page = "<?php echo site_url('kibe/add_usul_hapus')?>";
		var $dialog = $('<div></div>')
		               // .html('<iframe style="border: 0px; " src="' + page + '" width="100%" height="100%"></iframe>')
   		               .load(page)
		               .dialog({
		    title:"Usul Penghapusan Barang KIB E. Aset Tetap Lainnya",
		    modal: true,
		    draggable: false,
		    resizable: false,
		    position: ['center'],
			...
			buttons: {
				"Proses": function() {
				  ...
				  $.ajax({
					url : "<?php echo site_url('kibe/proses_usul_hapus')?>",
					type : 'POST',
					data : kode,
					success: function(msg){
...
$(function() {
		...
		var page = "<?php echo site_url('kibe/add_pindah_skpd')?>";
		var $dialog = $('<div></div>')
		               // .html('<iframe style="border: 0px; " src="' + page + '" width="100%" height="100%"></iframe>')
   		               .load(page)
		               .dialog({
			...
			buttons: {
				"Proses": function() {
				  ...
				  $.ajax({
					url : "<?php echo site_url('kibe/proses_pindah_skpd')?>",
					type : 'POST',
					data : kode,
					success: function(msg){
             	...
             	<div class="panel-content list-content">
                       ...
                       <div class="tab-content">
                            <div class="tab-pane list-pane active" id="tab1">
                                ...
                                <div class="pull-right">
                                    <form class="form-inline" method="POST" id="form_view" action=<?php echo base_URL(); ?>kibe/set/>
                                     ...
                                     <button class="btn"><i class="icon-search icon-large"></i></button>
                                        <a class="btn" href="<?php echo base_url(); ?>kibe/export"><i class="icon-ok-circle"></i> Excell</a>
                                    </form>
                                </div>
                	...
                	<div class="list-actions">
                            <div class="btn-toolbar">
                                ...
                                <div class="btn-group">
                                    <a href="<?php echo base_url() ?>kibe/add">
                                    ...
                                    <div class='btn-group'>
                                        ...
                                        <button class='btn dropdown-toggle' data-toggle='dropdown'>
                                            <span class='caret'></span>
                                                ...
                                                <ul class='dropdown-menu'><li>".anchor('kibe/lihat/'
			...
			if(empty($row->Penghapusan) || ($row->Penghapusan == 3)){
				...
				if($row->Penghapusan <> 3){
					if($this->session->userdata('lvl') == '01' || $this->session->userdata('lvl') == '02'){                                    
						echo "<li>".anchor('kibe/update/'.$row->Kd_Prov.'/'.$row->Kd_Kab_Kota.'/'.$row->Kd_Bidang.'/'.$row->Kd_Unit.'/'.$row->Kd_Sub.'/'.$row->Kd_UPB.'/'.$row->Kd_Aset1.'/'.$row->Kd_Aset2.'/'.$row->Kd_Aset3.'/'.$row->Kd_Aset4.'/'.$row->Kd_Aset5.'/'.$row->No_Register,"<i class='icon-pencil'></i>Ubah",array('class' => 'update'))."</li>";
					}
