...
class Lainnya extends CI_Controller {
	...
	function __construct(){
		parent::__construct();
		$this->auth->restrict();
		if(!$this->general->privilege_check('KIB_L',VIEW))
		    $this->general->no_access();
		$this->auth->clean_session('KIB_L');
	...
	{
		$data['form_cari']	= site_url('lainnya/cari');
		$data['link_kib']	= site_url('lainnya/listupb');
		...
		$data['link'] = array('link_add' => anchor('lainnya/add/','tambah data', array('class' => ADD)));
	...
	function listupb($bidang,$unit,$sub)
	{
		...
		$data['link_kib']	= site_url('lainnya/upb');
	...
	function upb($kd_bidang='',$kd_unit='',$kd_sub='',$kd_upb='')
	{
		...
		$data['query'] 		= $this->Lainnya_model->get_page($this->limit, $offset, $where, $like2." AND aktif is not null");
		$num_rows 			= $this->Lainnya_model->count_kib($where,$like2." AND aktif is not null")->Jumlah;
		$total 				= $this->Lainnya_model->count_kib($where,$like2." AND aktif is not null")->Total;
		...
		$data['tab2'] 		= site_url('lainnya/waiting/'.$kd_bidang.'/'.$kd_unit.'/'.$kd_sub.'/'.$kd_upb);
		$data['tab3'] 		= site_url('lainnya/skpd/'.$kd_bidang.'/'.$kd_unit.'/'.$kd_sub.'/'.$kd_upb);
		...
		{
			$data['message'] = 'Tidak ditemukan data Aset Lainnya !';
		}		
		$this->template->load('template','adminweb/lainnya/lainnya',$data);
	}
	...
	function skpd($kd_bidang='',$kd_unit='',$kd_sub='',$kd_upb='')
	{
		...
		$data['query'] 		= $this->Lainnya_model->get_page($this->limit, $offset, $where, $like2);
		$num_rows 			= $this->Lainnya_model->count_kib($where,$like2)->Total;
		$total 				= $this->Lainnya_model->count_kib($where,$like2)->Jumlah;
		$harga_total		= number_format($total,0,",",".");
		...
		$data['tab1'] 		= site_url('lainnya/upb/'.$kd_bidang.'/'.$kd_unit.'/'.$kd_sub.'/'.$kd_upb);
		$data['tab2'] 		= site_url('lainnya/waiting/'.$kd_bidang.'/'.$kd_unit.'/'.$kd_sub.'/'.$kd_upb);
		...
		{
			$data['message'] = 'Tidak ditemukan data Aset Lainnya  yang belum di periksa!';
		}		
		$this->template->load('template','adminweb/lainnya/lainnya_skpd',$data);
	}
	...
	function waiting($kd_bidang='',$kd_unit='',$kd_sub='',$kd_upb='')
	{
		...
		$nmupb				= $this->Ref_upb_model->nama_upb($kd_bidang,$kd_unit,$kd_sub,$kd_upb);
		$data['query'] 		= $this->Lainnya_model->get_page($this->limit, $offset, $where, $like2." AND aktif is null");
		$num_rows 			= $this->Lainnya_model->count_kib($where,$like2." AND aktif is null")->Jumlah;
		$total 				= $this->Lainnya_model->count_kib($where,$like2." AND aktif is null")->Total;
		$harga_total		= number_format($total,0,",",".");
		...
		$this->template->load('template','adminweb/lainnya/lainnya_nonaktif',$data);
	}
	...
	{
		if(!$this->general->privilege_check('KIB_L',ADD))
		    $this->general->no_access();
		...
		$data['title']        = $this->title;
		$data['option_bidang'] = $this->Model_chain->getBidangList();
		$data['form_action']  = site_url('lainnya/save_riwayat');
		$data['link']         = array('link_back' => anchor('lainnya','kembali', array('class' => 'back')));
		...
		$this->template->load('template','adminweb/lainnya/lainnya_addriwayat',array_merge($data,$get));
	}
	...
	function save_riwayat()
	{
		...
		$arr = array(
			...
			// print_r($arr); exit();
			$sql = $this->Lainnya_model->add_riwayat($arr);
			
			if ($sql){
				$this->session->set_flashdata('message', 'Satu data Riwayat Aset Lainnya berhasil ditambah!, Silahkan tunggu verifikasi oleh admin');
				redirect($this->session->userdata('last_url'));
			}else{		
				$this->session->set_flashdata('message', 'Satu data Riwayat Aset Lainnya Gagal ditambah!');
				redirect($this->session->userdata('last_url'));
				}		
	}
	...
	function lihat($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register)
	{
		...
		$jumlah = $this->Lainnya_model->get_lainnya_by_id($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register)->num_rows();
																				  										  
												  
		if ($jumlah > 0){
			$this->session->set_userdata('last_url', current_url());
			$lainnya = $this->Lainnya_model->get_lainnya_by_id($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register)->row();
												  
			$namaaset= $this->Ref_rek_aset5_model->nama_aset($kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5);
			
				$this->session->set_userdata('Kd_Aset1', $lainnya->Kd_Aset1);
				$this->session->set_userdata('Kd_Aset2', $lainnya->Kd_Aset2);
				$this->session->set_userdata('Kd_Aset3', $lainnya->Kd_Aset3);
				$this->session->set_userdata('Kd_Aset4', $lainnya->Kd_Aset4);
				$this->session->set_userdata('Kd_Aset5', $lainnya->Kd_Aset5);
				$this->session->set_userdata('No_Register', $lainnya->No_Register);
				
				$data['default']['Kd_Prov']       = $lainnya->Kd_Prov;
				$data['default']['Kd_Kab_Kota']   = $lainnya->Kd_Kab_Kota;				
				$data['default']['Kd_Bidang']     = $lainnya->Kd_Bidang;  
				$data['default']['Kd_Unit']       = $lainnya->Kd_Unit;
				$data['default']['Kd_Sub']        = $lainnya->Kd_Sub;
				$data['default']['Kd_UPB']        = $lainnya->Kd_UPB;
				
				$data['default']['Kd_Aset1']      = $lainnya->Kd_Aset1;
				$data['default']['Kd_Aset2']      = $lainnya->Kd_Aset2;
				$data['default']['Kd_Aset3']      = $lainnya->Kd_Aset3;
				$data['default']['Kd_Aset4']      = $lainnya->Kd_Aset4;
				$data['default']['Kd_Aset5']      = $lainnya->Kd_Aset5;
				$data['default']['Nm_Aset5']      = $namaaset;
				$data['default']['No_Register']   = $lainnya->No_Register;
				$data['default']['Kd_Pemilik']    = $lainnya->Kd_Pemilik;
				$data['default']['Judul']         = $lainnya->Judul;
				$data['default']['Pencipta']      = $lainnya->Pencipta;
				$data['default']['Bahan']         = $lainnya->Bahan;
				$data['default']['Ukuran']        = $lainnya->Ukuran;
				$data['default']['Tgl_Perolehan'] = $lainnya->Tgl_Perolehan;
				$data['default']['Tgl_Pembukuan'] = $lainnya->Tgl_Pembukuan;
				$data['default']['Asal_usul']     = $lainnya->Asal_usul;
				$data['default']['Kondisi']       = $lainnya->Kondisi;
				$data['default']['Harga']         = $lainnya->Harga;
				$data['default']['Keterangan']    = $lainnya->Keterangan;
				$data['default']['Nilai_Sisa']    = $lainnya->Nilai_Sisa;
				$data['default']['Masa_Manfaat']  = $lainnya->Masa_Manfaat; 
				
				$data['query'] 		= $this->Lainnya_model->data_foto($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,
												  $kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register);
								
								
				$data['riwayat'] 	= $this->Lainnya_model->get_riwayat_barang($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,
												  $kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register);

					

				$this->template->load('template','adminweb/lainnya/lainnya_lihat',$data);				
		}else{
			echo "tidak ada data";	
		}
	}
	...
	function export(){		
		...
		$arg = explode('/', $curl);
		if ($arg[5] == 'skpd' ) {
			$d['data_view'] = $this->db->query("select * from Ta_Lainnya inner join Ref_Rek_Aset5 on 
...
a.Kd_Aset5=Ref_Rek_Aset5.Kd_Aset5 WHERE $where $like2 AND aktif is null");	
		...
		$this->load->view('adminweb/lainnya/export',$d);
	...
	function last_id($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register){
			...
			$array_keys_values = $this->db->get_where('Ta_FotoLainnya',array('Kd_Prov' => $kd_prov,'Kd_Kab_Kota' => $kd_kab,'Kd_Bidang' => $kd_bidang,
	...
	function last_Kd_Id($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register){
			...
			$array_keys_values = $this->db->get_where('Ta_KILER',array('Kd_Prov' => $kd_prov,'Kd_Kab_Kota' => $kd_kab,'Kd_Bidang' => $kd_bidang,
			'Kd_Unit' => $kd_unit,'Kd_Sub' => $kd_sub,'Kd_UPB' => $kd_upb,'Kd_Aset1' => $kd_aset1,'Kd_Aset2' => $kd_aset2,'Kd_Aset3' => $kd_aset3,
			'Kd_Aset4' => $kd_aset4,'Kd_Aset5' => $kd_aset5,'No_Register' => $no_register));
       		foreach ($array_keys_values->result() as $data) {
					 $result=$data->Kd_Id+1;
	...
	function last_id_hapus($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register){
			...
			$this->db->select_MAX('Kd_Id');
			$array_keys_values = $this->db->get_where('Ta_KILHapus',array('Kd_Prov' => $kd_prov,'Kd_Kab_Kota' => $kd_kab,'Kd_Bidang' => $kd_bidang,
			'Kd_Unit' => $kd_unit,'Kd_Sub' => $kd_sub,'Kd_UPB' => $kd_upb,'Kd_Aset1' => $kd_aset1,'Kd_Aset2' => $kd_aset2,'Kd_Aset3' => $kd_aset3,
			'Kd_Aset4' => $kd_aset4,'Kd_Aset5' => $kd_aset5,'No_Register' => $no_register));
       		foreach ($array_keys_values->result() as $data) {
					 $result=$data->Kd_Id+1;
	...
	public function upload()
	{
		if(isset($_FILES['image']))
		{
			...
			{
				$config['upload_path']   = './assets/uploads_lainnya';
				...
				if($this->multi_upload->do_upload($data2[$no]))
				{
					...
					$foto = array(
						...
						$this->Lainnya_model->insert($foto);
						
					echo img(array(
						'src'=>base_url("assets/uploads_lainnya/$image_data[file_name]"),
						'width'=>179,
    ...
    $row = $this->db->get_where('Ta_FotoLainnya',array('Kd_Prov' => $kd_prov,'Kd_Kab_Kota' => $kd_kab_kota,'Kd_Bidang' => $kd_bidang,'Kd_Unit' => $kd_unit,
     ...
     unlink("./assets/uploads_lainnya/$row->Nama_foto");

    $this->db->delete('Ta_FotoLainnya', array('Kd_Prov' => $kd_prov,'Kd_Kab_Kota' => $kd_kab_kota,'Kd_Bidang' => $kd_bidang,'Kd_Unit' => $kd_unit,
		'Kd_Sub' => $kd_sub,'Kd_UPB' => $kd_upb,'Kd_Aset1' => $kd_aset1,'Kd_Aset2' => $kd_aset2,'Kd_Aset3' => $kd_aset3,
	...
	/**
	 * Hapus data Aset Lainnya
	 */
	function hapus($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register)
	{
		if(!$this->general->privilege_check('KIB_L','remove'))
		    $this->general->no_access();

		$this->Lainnya_model->hapus($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register);
		redirect('lainnya');
	}
	...
	function ajax_hapus(){
		...
		$this->Lainnya_model->hapus($kd_prov,$kd_kab_kota,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1x,
								 $kd_aset2x,$kd_aset3x,$kd_aset4x,$kd_aset5x,$no_register);	
	}

	function hapus_riwayat(){
		if(!$this->general->privilege_check('RIWAYAT_KIB_L',REMOVE))
		    $this->general->no_access();
		...
		$this->Lainnya_model->hapus_riwayat($kd_riwayat,$kd_id,$kd_prov,$kd_kab_kota,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1x,
								 $kd_aset2x,$kd_aset3x,$kd_aset4x,$kd_aset5x,$no_register);	
	}
...
function pindahskpd()
	{
		...
		if ($this->session->userdata('lvl') == 01){
			...
			$register 	= $this->Lainnya_model->get_last_noregister($this->input->post('tkd_bidang'),$this->input->post('tkd_unit'),$this->input->post('tkd_sub'),$this->input->post('tkd_upb'),$kd_aset1x,$kd_aset2x,$kd_aset3x,$kd_aset4x,$kd_aset5x);
	...
	$lainnya = array(
		...
		$this->Lainnya_model->sm_update($kd_prov,$kd_kab_kota,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1x,
								  $kd_aset2x,$kd_aset3x,$kd_aset4x,$kd_aset5x,$no_register,$lainnya);	
	...
	function aktifkan(){
		...
		$this->Lainnya_model->sm_update($kd_prov,$kd_kab_kota,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,
	...
	/**
	 * Pindah ke halaman tambah lainnya
	 */
	function add()
	{	
		if(!$this->general->privilege_check('KIB_L',ADD))
		...
		$data['title'] 			= $this->title;
		$data['h2_title'] 		= 'Aset Lainnya > Tambah Data';
		$data['form_action']	= site_url('lainnya/add_process');
		$data['link'] 			= array('link_back' => anchor('lainnya','kembali', array('class' => 'back'))
										);
		...
		$tahun 		= $this->session->userdata('tahun_anggaran');
		$data['option_ruang']   = $this->Ta_ruang_model->RuangList($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$tahun); 
		$this->template->load('template','adminweb/lainnya/lainnya_addform',$data);
	}
	
	/**
	 * Proses tambah data lainnya
	 */
	...
	{		
			...
			if ($jp !=""){
						...
						$this->session->set_flashdata('message', $no.' data Aset Lainnya berhasil ditambah!');
						redirect('lainnya/upb/'.$this->input->post('Kd_Bidang').'/'.$this->input->post('Kd_Unit').'/'
						...
						$sql = $this->Lainnya_model->add($lainnya);
						$this->session->set_flashdata('message', 'Satu data Aset Lainnya berhasil ditambah!, Silahkan tunggu verifikasi oleh admin');
						redirect('lainnya/upb/'.$this->input->post('Kd_Bidang').'/'.$this->input->post('Kd_Unit').'/'
										.$this->input->post('Kd_Sub').'/'.$this->input->post('Kd_UPB'));
						}
			
						if ($sql == false){		
							$this->session->set_flashdata('message', 'Satu data Aset Lainnya Gagal ditambah!');
							redirect('lainnya/upb/'.$this->input->post('Kd_Bidang').'/'.$this->input->post('Kd_Unit').'/'
										.$this->input->post('Kd_Sub').'/'.$this->input->post('Kd_UPB'));
							}
	...
	{
		if(!$this->general->privilege_check('KIB_L',EDIT))
		    $this->general->no_access();
		...
		$jumlah = $this->Lainnya_model->get_lainnya_by_id($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,
												  $kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register)->num_rows();
																				  										  
												  
		if ($jumlah > 0){
			$lainnya = $this->Lainnya_model->get_lainnya_by_id($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,
			...
			$this->session->set_userdata('No_Register', $lainnya->No_Register);   
				...
				$data['default']['Nm_Aset5'] 			= $namaaset;
				$data['default']['No_Register'] 		= $lainnya->No_Register;
				$data['default']['Kd_Ruang'] 			= $lainnya->Kd_Ruang;
				$data['default']['Kd_Pemilik'] 			= $lainnya->Kd_Pemilik;
				$data['default']['Judul'] 				= $lainnya->Judul;
				$data['default']['Pencipta'] 			= $lainnya->Pencipta;
				$data['default']['Bahan'] 				= $lainnya->Bahan;
				...
				$this->template->load('template','adminweb/lainnya/lainnya_updateform',$data);				
		}else{
			redirect('lainnya');	
		}
	}
	...
	function update_process()
	{
			...
			$sql = $this->Lainnya_model->sm_update($kd_prov,$kd_kab_kota,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,
												$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register,$lainnya);	
			if ($sql){
	...
	function lookup()
	{
		...
		$count = $this->Ref_rek_aset5_model->count_lainnya($where);
		...
		$data1 = $this->Ref_rek_aset5_model->get_lainnya($where, $sidx, $sord, $limit, $start)->result();
	...
	function json(){
        ...
        $data['response'] = 'false'; 
        $query = $this->Ref_rek_aset5_model->json_lainnya($keyword); 
        if( ! empty($query) )
        {
            $data['response'] = 'true'; 
            $data['message'] = array(); 
...
$num_rows = $this->Lainnya_model->get_last_noregister($kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1x,$kd_aset2x,$kd_aset3x,$kd_aset4x,$kd_aset5x);		
	...
	function cek_register(){
			...
			$num_rows = $this->Lainnya_model->cek_register($kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1x,$kd_aset2x,$kd_aset3x,$kd_aset4x,$kd_aset5x,$no_registerx)->num_rows();
			echo $num_rows;
			}
	
	/**
	 * Cek apakah $id_lainnya valid, agar tidak ganda
	 */
	function valid_no_register($no_register)
	{
		if ($this->Lainnya_model->no_register($no_register) == TRUE)
		{
			$this->form_validation->set_message('valid_id', "lainnya dengan Kode $id_lainnya sudah terdaftar");
			return FALSE;
		}
		else
		{			
	...
	/**
	 * Cek apakah $id_lainnya valid, agar tidak ganda. Hanya untuk proses update data lainnya
	 */
	function cek_register2()
	{
		...
		{
if($this->Lainnya_model->valid_no_register($kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register) === TRUE)
			{
				$this->form_validation->set_message('valid_no_register2', "lainnya dengan kode $new_id sudah terdaftar");
				return FALSE;
			}
			else
			{
	...
	function usul_guna(){
		...
		$this->Lainnya_model->usul_guna($kd_prov,$kd_kab_kota,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1x,$kd_aset2x,$kd_aset3x,$kd_aset4x,$kd_aset5x,$no_register);	
	}

	function add_usul_hapus(){
		$data['form_action']   = site_url('lainnya/usul_hapus');
	...
	function proses_usul_hapus(){
		...
		if ($this->session->userdata('lvl') == 01){
			...
			$sql = $this->Lainnya_model->insert_usul_penghapusan($arr_data);	
			if ($sql)
				return true;
	}
	...
	{
		if(!$this->general->privilege_check('RIWAYAT_KIB_L',EDIT))
		    $this->general->no_access();

		$data['title']       = $this->title;
		$data['h2_title']    = 'RIWAYAT KIB LAINNYA > Update';
		$data['form_action'] = site_url('lainnya/update_riwayat_process');
		$data['link']        = array('link_back' => anchor('lainnya','kembali', array('class' => 'back'))
		);
		$data['option_q']    = $this->Model_chain->getRiwayatList();
		...
		$dat = $this->Lainnya_model->get_lainnya_by_id($kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register)->row_array();
		if(!$dat)
	        show_error("Tidak ada data!");
	    
	    $data['HargaKoreksi']      = $dat['HargaKoreksi'];

		$get = $this->Lainnya_model->get_kiler_by_id($kd_riwayat,$kd_id,$kd_prov,$kd_kab,$kd_bidang,$kd_unit,$kd_sub,$kd_upb,$kd_aset1,$kd_aset2,$kd_aset3,$kd_aset4,$kd_aset5,$no_register)->row_array();
		if(!$get)
	        show_error("Tidak ada data!");
		...
		$this->template->load('template','adminweb/lainnya/lainnya_updateriwayat',array_merge($data,$get));
	...
	function update_riwayat_process()
	{
		...
		$arr = array(
			...
			$sql = $this->Lainnya_model->update_riwayat($arr);
...
/* Location: ./system/application/controllers/lainnya.php */