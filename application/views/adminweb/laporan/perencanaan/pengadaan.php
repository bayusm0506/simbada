<?php
	...
	$this->load->library('inventarisasi/PDF_MC_Table_pengadaan');
	...
	$pdf->MultiCell(435,5,"DAFTAR PENGADAAN \n ".NM_PEMDA,0,'C');
	...
	if ($jumlah_pengadaan > 0){
		foreach ($pengadaan->result() as $row){
			...
			$pdf->Row(array($no,  $row->Nm_Aset5, $tgl_kontrak, strtoupper($row->No_Kontrak), $tgl_kuitansi,  $row->No_Kuitansi, rp($jlh), $harga_satuan,$jumlah, "-", $unit, $row->Keterangan));
