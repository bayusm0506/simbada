...
<script>
	...
	if(confirm("Yakin data yang dipilih akan dihapus dalam usulan penghapusan ??")){	
		...
		$("input.checkbox1:checked").each(function(){
			...
			$.ajax({
					type: "POST",
					url: "<?php echo base_url(); ?>penghapusan/ajax_hapus_d",
					data:kode,
					cache: false,
...
$(".batal_usulan").click(function(event) {
	...
	if(confirm("Yakin data ini akan dihapus dalam usulan penghapusan ??"))
	{
		...
		$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>penghapusan/hapus_usul_d",
				data:kode,
			 	cache: false,
			 	success: function(html){
			 		$("#listhapus_"+no).fadeTo(300,0, function() {
...
$(".tolak_usulan").click(function(event) {
	...
	if(confirm("Anda yakin data ini akan menolak data ini??"))
	{
		...
		$.ajax({
				type: "POST",
				url: "<?php echo base_url() ?>penghapusan/proses_usul_d",
				data:kode,
			 	cache: false,
			 	success: function(html){
			 		$("#listhapus_"+no).fadeTo(300,0, function() {
...
<a class="a-link" href="'.site_url('penghapusan/rincian_kibd/'.encrypt_url($sk_tmp)).'"> Daftar Penghapusan </a> <span class="pull-right">SK PENGHAPUSAN : ' . strtoupper($sk_tmp) . '</span></p>': '';?>
                ...
                <div class="panel">
             	...
             	<div class="panel-content list-content">
                        ...
                        <ul class="nav nav-tabs">
                            ...
                            <a href="<?php echo ! empty($kibd) ?  $kibd: '#'; ?>" class="tab-page" data-toggle="tab">KIB D
                             <span class="badge"><?php echo ! empty($jumlah) ? $jumlah : '0';?></span></a></li>
                            
                            <li <?php if ($this->uri->segment(2) == 'kibe' ) {echo "class='active'";}?>>
                            <a href="<?php echo ! empty($kibe) ?  $kibe: '#'; ?>" class="tab-page" data-toggle="tab">KIB E</a></li>
                      ...
                      echo "<tr id='listhapus_$i'>
                          ...
                          <td>
                                            <div class='btn-group'>
                                                ...
                                                <ul class='dropdown-menu'><li>".anchor(current_url().'#',"<i class='icon-retweet'></i>Batalkan Usulan",array('class'=> 'batal_hapus','Kd_Prov' => $row->Kd_Prov,'Kd_Kab_Kota' => $row->Kd_Kab_Kota,'Kd_Bidang' => $row->Kd_Bidang,'Kd_Unit' => $row->Kd_Unit,'Kd_Sub' => $row->Kd_Sub,'Kd_UPB' => $row->Kd_UPB,'Kd_Aset1' => $row->Kd_Aset1,'Kd_Aset2' => $row->Kd_Aset2,'Kd_Aset3' => $row->Kd_Aset3,'Kd_Aset4' => $row->Kd_Aset4,'Kd_Aset5' => $row->Kd_Aset5,'No_Register' => $row->No_Register,'no'=>$i))."</li>
												</ul>
                                            </div>
                                        </td>
                      </tr>";
