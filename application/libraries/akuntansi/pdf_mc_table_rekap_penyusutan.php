...
class PDF_MC_Table_rekap_penyusutan extends FPDF
{
...
function UPBTitle2($kode_lokasi)
{
	...
	function Rowheader($awal,$akhir)
	{
			...
			$this->SetXY($x+10,$y2+3);
			$this->MultiCell(15,5,"No.",0,'C');

			$this->SetXY($x+25,$y);
			$this->MultiCell(85,5,"Akun Neraca",1,'C');
			$this->SetXY($x+25,$y2+3);
			$this->MultiCell(35,10,"Kode Akun",1,'C');
			$this->SetXY($x+60,$y2+3);
			$this->MultiCell(50,10,"Bidang Barang",1,'C');


			$this->SetXY($x+110,$y);
			$this->MultiCell(105,5,"Bidang Barang",1,'C');
			$this->SetXY($x+110,$y2+3);
			$this->MultiCell(35,10,"Kode Barang",1,'C');
			$this->SetXY($x+145,$y2+3);
			$this->MultiCell(70,10,"Sub Bidang Barang",1,'C');

			$this->SetXY($x+215,$y);
			$this->MultiCell(235,5,"Jumlah",1,'C');
			$this->SetXY($x+215,$y2+3);
			$this->MultiCell(47,5,"Nilai Perolehan",1,'C');
			$this->SetXY($x+215,$y2+8);
			$this->MultiCell(47,5,$akhir,1,'C');

			$this->SetXY($x+262,$y2+3);
			$this->MultiCell(47,5,"Nilai Buku Awal",1,'C');
			$this->SetXY($x+262,$y2+8);
			$this->MultiCell(47,5,$awal,1,'C');

			$this->SetXY($x+309,$y2+3);
			$this->MultiCell(47,5,"Akumulasi Penyusutan",1,'C');
			$this->SetXY($x+309,$y2+8);
			$this->MultiCell(47,5,$akhir,1,'C');

			$this->SetXY($x+356,$y2+3);
			$this->MultiCell(47,5,"Kapitalisasi",1,'C');
			$this->SetXY($x+356,$y2+8);
			$this->MultiCell(47,5,$akhir,1,'C');

			$this->SetXY($x+403,$y2+3);
			$this->MultiCell(47,5,"Nilai Buku Akhir",1,'C');
			$this->SetXY($x+403,$y2+8);
			$this->MultiCell(47,5,$akhir,1,'C');
	}
...
function ttd2($nm_pimpinan,$nip_pimpinan,$jbt_pimpinan,$nm_pengurus,$nip_pengurus,$tanggal)
{
	...
	function Header()
	{
	    ...
	    $this->Cell(0,9,'DAFTAR PENYUSUTAN KIB B. PERALATAN DAN MESIN',0,0,'C');
		$this->Ln();
		$this->SetFont('Arial','B','10');
